...
AP <- subset(
    x = AP ,
    subset = nMonth > max(nMonth,na.rm=T)-18 )
...
<<TestFeiring,include=FALSE>>=
conf$disp <- "Feiring"
AP <- AP[ which(AP$Sykehusnavn=="Feiring"&AP$nMonth>24) , ]
SO <- SO[ which(SO$Sykehusnavn=="Feiring"&SO$nMonth>24) , ]
NSTEMI <- NSTEMI[ which(NSTEMI$Sykehusnavn=="Feiring"&NSTEMI$nMonth>24) , ]
@ 
...
\begin{frame}[fragile]
<<FigNIndikasjon,fig.cap="Antall primær prosedyrer etter indikasjon og PCI senter",echo=FALSE,fig.width=8,fig.height=4,out.width="\\textwidth">>=
ggplot(
    data = AP ,
    mapping = aes(
        x = Month ,
...
TAB1 <- addmargins(
    xtabs(
        formula = ~ Month + Skjemanavn ,
        data = SO))

print.xtable(
    x = xtable(
        x = TAB1 ,
        caption = "Antall registreringer etter skjema og måned" ,
        digits = 0) ,
    booktabs = TRUE )
@   
\end{frame}
...
\begin{frame}[fragile]
<<SOtab1b, echo = FALSE , results = 'asis'>>=
TAB1b <- addmargins(
    xtabs(
        formula = ~ Month + Skjemanavn ,
        subset = SkjemaStatus == 1 ,
        data = SO))

print.xtable(
    x = xtable(
        x = TAB1b ,
        caption = "Antall ferdigstilte registreringer etter skjema og måned" ,
        digits = 0) ,
    booktabs = TRUE )
@   
\end{frame}
...
print.xtable(
    x = xtable(
        x = TAB2 ,
        caption = "Prosent andel ferdigstilte registreringer etter skjema og måned" ,
        digits = 1) ,
    booktabs = TRUE )
@   
\end{frame}
...
\begin{frame}[fragile]
<<FigT2TetterIndikasjon,fig.cap="Fordeling av ventetid fra indeks innleggelse til prosedyre for henviste pasienter etter indikasjon", fig.height = 4, fig.width = 8, out.width = "\\textwidth", warning = FALSE, echo = FALSE>>=
ggplot(
    data = AP ,
    mapping = aes(
