...
if (params$rendered_by_shiny == TRUE)
    shiny::setProgress(0.1)  # set progress to 35%
```
...
```{r}
if (params$rendered_by_shiny == TRUE)
    shiny::setProgress(0.2)  # set progress to 35%
```
...
t_timeTableMonth_fjor <- data.frame(t(timeTableMonth_fjor[-1]))
colnames(t_timeTableMonth_fjor) <- timeTableMonth_fjor[, 1]
...
t_timeTableMonth_aar <- data.frame(t(timeTableMonth_aar[-1]))
colnames(t_timeTableMonth_aar) <- timeTableMonth_aar[, 1] 
...
```{r}
if (params$rendered_by_shiny == TRUE)
    shiny::setProgress(0.3)  # set progress to 35%
```
...
```{r}
if (params$rendered_by_shiny == TRUE)
    shiny::setProgress(0.4)  # set progress to 35%
```
...
```{r}
if (params$rendered_by_shiny == TRUE)
    shiny::setProgress(0.5)  # set progress to 35%
```
<br/><br/>
...
TabAngiografor_fjor <- tab_angiografor %>%
  select(forkortetNavnAngiografor,
         colnames(t_timeTableMonth_fjor))%>%
  mutate(tot_fjor = rowSums(across(where(is.numeric))))

TabAngiografor_aar <- tab_angiografor %>%
  select(forkortetNavnAngiografor,
         colnames(t_timeTableMonth_aar)) %>%
  mutate(tot_aar = rowSums(across(where(is.numeric))))


TabAngiografor <- full_join(
  TabAngiografor_fjor,
  TabAngiografor_aar,
  by = "forkortetNavnAngiografor") %>%
  as.data.frame() %>%
  janitor::adorn_totals(., where = "row", name = "Totalt")

colnames(TabAngiografor)[14] <- paste0("Totalt i ", periode_data$sisteHeleYear)
colnames(TabAngiografor)[ncol(TabAngiografor)] <- paste0("Totalt hittil i ", periode_data$nyesteRegYear)

TabAngiografor[TabAngiografor == 0] <- " - "
TabAngiografor[is.na(TabAngiografor)] <- " - "

align <- c(rep("r", ncol(TabAngiografor)))

if (params$tableFormat == "html") {
  TabAngiografor %>% rename(" " = "forkortetNavnAngiografor") %>%
    ...
    kableExtra::row_spec(row = 0, font_size = 10, bold = TRUE,
                         angle = -45, align = "c") %>%
    kableExtra::row_spec(row = nrow(TabAngiografor),
                         background = "#EBEBEB") %>%
    kableExtra::column_spec(column = 14,
                            background = "#EBEBEB") %>%
    kableExtra::column_spec(column = ncol(TabAngiografor),
                            background = "#EBEBEB") %>%
    kableExtra::add_header_above(
      ...
      header = c("",
                 ...
                 setNames((ncol(TabAngiografor)-14),
  ...
  TabAngiografor %>% rename(" " = "forkortetNavnAngiografor") %>% 
    ...
    kableExtra::row_spec(row = nrow(TabAngiografor)-1, hline_after = TRUE) %>%
    kableExtra::add_header_above(
      .,
      header = c("",
                 setNames(13,
...
TabPciOperator_fjor <- tab_pciOperator %>% 
  select(forkortetNavnPCIoperator,
         colnames(t_timeTableMonth_fjor))%>% 
  mutate(tot_fjor = rowSums(across(where(is.numeric))))


TabPciOperator_aar <- tab_pciOperator %>% 
  select(forkortetNavnPCIoperator,
         colnames(t_timeTableMonth_aar)) %>% 
  mutate(tot_aar = rowSums(across(where(is.numeric))))

TabPciOperator <- full_join(
  TabPciOperator_fjor,
  TabPciOperator_aar,
  by = "forkortetNavnPCIoperator") %>% 
  as.data.frame() %>% 
  janitor::adorn_totals(., where = "row", name = "Totalt")

colnames(TabPciOperator)[14] <- paste0("Totalt i ", periode_data$sisteHeleYear)
colnames(TabPciOperator)[ncol(TabPciOperator)] <- paste0("Totalt hittil i ", periode_data$nyesteRegYear)

TabPciOperator[TabPciOperator == 0] <- " - "
TabPciOperator[is.na(TabPciOperator)] <- " - "

align <- c(rep("r", ncol(TabPciOperator)))

if (params$tableFormat == "html") {
  TabPciOperator %>% rename(" " = "forkortetNavnPCIoperator") %>%
    knitr::kable(caption = cap,
                 align = align) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped",
                                                    "hover",
    ...
    kableExtra::row_spec(row = 0, font_size = 10, bold = TRUE,
                         angle = -45, align = "c") %>%
    kableExtra::row_spec(row = nrow(TabPciOperator),
                         background = "#EBEBEB") %>%
    kableExtra::column_spec(column = 14,
                            background = "#EBEBEB") %>%
    kableExtra::column_spec(column = ncol(TabPciOperator),
                            background = "#EBEBEB") %>%
    kableExtra::add_header_above(
      .,
      header = c("",
                 setNames(13,
                          periode_data$sisteHeleYear),
                 setNames((ncol(TabPciOperator)-14),
                          periode_data$nyesteRegYear)),
      bold = TRUE,
      font_size = 12)
  ...
  TabPciOperator %>% rename(" " = "forkortetNavnPCIoperator") %>% 
    ...
    kableExtra::kable_styling(latex_options = c("HOLD_position",
                              ...
                              font_size = 8) %>%
    kableExtra::row_spec(row = 0, angle = 90) %>%
    kableExtra::row_spec(row = nrow(TabPciOperator)-1, hline_after = TRUE) %>%
    kableExtra::add_header_above(
      .,
      header = c("",
                 setNames(13,
