...
# construct a closure to create rounded quantiles 
...
ReportYear <- max( AP$Year , na.rm = TRUE )
@ 
...
Hmisc::latex(
    ...
    center = "none" ,
    caption = "Ventetid i dager for NSTEMI prosedyrer etter henvisningsstatus over de siste 18 måneder" ,
    title = "",
    file = "")
...
Dtmp <- subset(
    x = NSTEMI ,
    subset = (OverflyttetFraSykehus %in% c( "Haraldsplass sykehus", "Voss sjukehus", "Haugesund sykehus", "Stord sykehus", "Førde sentralsjukehus", "Lærdal sjukehus", "Odda sykehus", "Nordfjordeid sjukehus")) & (Year %in% 2013:2016 ) ,
    select = c(DelayFromIndexAdmissionToProcedure,OverflyttetFraSykehus) 
    )
...
legend(
    ...
    bty = "n" ,
    title = "Timer" ,
    legend = levels(Dtmp$DelayFromIndexAdmissionToProcedure )[3:1],
    fill = pal[3:1] )
...
Dtmp <- subset(
    x = NSTEMI ,
    subset = (OverflyttetFraSykehus %in% c( "Haraldsplass sykehus", "Voss sjukehus", "Haugesund sykehus", "Stord sykehus", "Førde sentralsjukehus", "Lærdal sjukehus", "Odda sykehus", "Nordfjordeid sjukehus")) & (Year == ReportYear ) ,
    select = c(DelayFromIndexAdmissionToProcedure,OverflyttetFraSykehus) 
    )
