...
SSQuery <- paste0("
SELECT 
  AvdRESH, ForlopsID, ProsedyreDato, Indikasjon, ProsedyreType,  Stentnavn, StentType
FROM SegmentStent
WHERE 
...
APQuery <- paste0("
SELECT
  AvdRESH, ForlopsID, ProsedyreType, ProsedyreDato
FROM
  AngioPCIVar
WHERE
  ProsedyreType  != 'Angio' AND
  ProsedyreDato >= '", periode_data$forste_dato, "' AND
  ProsedyreDato <= '", periode_data$siste_dato, "' 
 ")
...
AP %<>%
  noric::legg_til_antall_stent(., df_ss = SS) %>% 
  dplyr::mutate(Nstents = dplyr::if_else(
    condition = is.na(.data$ant_stent), 
    true = 0, 
    false = .data$ant_stent))
