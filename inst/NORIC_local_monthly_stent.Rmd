...
Antall stenter per prosedyre i perioden `r paste0( range(PCImedStent$ProsedyreDato) , collapse = " til ")`, min. 1 stent (N = `r sum(PCImedStent$Nstents>0)`)
...
# Siste showN måneder, vise alle brukte stenter. ikke vise historiske stenter 
SS %<>% mutate(nMonth = as.numeric(as.factor(format(ProsedyreDato,format = "%y-%m"))))
...
cap <- "Antall (prosent) stenter etter merke og måned"
xtabs(formula = ~ YearMonth + Stentnavn,
      data = SSpci,
