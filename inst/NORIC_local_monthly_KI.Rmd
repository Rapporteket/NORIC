...
# SQL SPØRRING ETTER NASJONALE DATA : 
# Are?
# dbType <- "mysql"
# SOQuery <- "SELECT * FROM SkjemaOversikt"
# SSQuery <- "SELECT * FROM SegementStent"
# APQuery <- "SELECT * FROM AngioPCIVar"
# AKQuery <- "SELECT * FROM AortaklaffVar"

# sO_nasjonalt <- rapbase::LoadRegData(params$registryName, SOQuery, dbType)
# sS_nasjonalt <- rapbase::LoadRegData(params$registryName, SSQuery, dbType)
# aP_nasjonalt <- rapbase::LoadRegData(params$registryName, APQuery, dbType)
# aK_nasjonalt <- rapbase::LoadRegData(params$registryName, AKQuery, dbType)

```


```{r yearTabPrepare, include=FALSE, eval=TRUE}
sisteDatoYear <- lubridate::floor_date(as.Date(Sys.Date()), "year")-1
yearDato <- seq(as.Date("2017-01-01"), sisteDatoYear, by = "year")

timeTableYear <- as.data.frame(yearDato) %>%
  dplyr::mutate(Year = as.numeric(format(.data$yearDato, format = "%Y"))) %>%
  dplyr::select(.data$Year)
```


```{r kvartalTabPrepare, include=FALSE, eval=TRUE}
sisteDatoKvartal <- lubridate::floor_date(as.Date(Sys.Date()), "quarter") - 1
forsteDatoKvartal <- lubridate::floor_date(sisteDatoKvartal - 450, "quarter")
kvartalDato <- seq(forsteDatoKvartal, sisteDatoKvartal, by = "quarter")

timeTableQuarter <- as.data.frame(kvartalDato) %>%
  dplyr::mutate(
    Kvartal = lubridate::quarter(x = .data$kvartalDato, with_year = FALSE),
    Year = lubridate::year(x = .data$kvartalDato),
    Kvartal = paste0(Year, " Q", Kvartal)
    ) %>%
  dplyr::select(.data$Kvartal)
```

```{r monthTabPrepare, include=FALSE, eval=TRUE}
sisteDatoMonth <- lubridate::floor_date(as.Date(Sys.Date()), "month") - 1
forsteDatoMonth <- lubridate::floor_date(sisteDatoMonth - 420, "month") 
monthDato <- seq(forsteDatoMonth, sisteDatoMonth, by = "month")

timeTableMonth <- as.data.frame(monthDato) %>%
  dplyr::mutate(Month = as.factor(format(x = .data$monthDato, 
                                         format = "%Y-%m"))) %>%
...
sO_nasjonalt %<>% dplyr::filter(
  ! (.data$Sykehusnavn %in%  c("AHUS Gardermoen", "Rikshospitalet") &
       .data$Year < 2015))
...
# Ventetid - NSTEMI
aP_nasjonalt %<>% dplyr::mutate(
  AdmissionType = dplyr::recode(
    .data$OverflyttetFra,
    "Annet sykehus" = "Referred",
    .missing = NA_character_,
    "Annen  avdeling på sykehuset" = NA_character_,
...
aK_nasjonalt  %<>% 
  dplyr::filter(RegDate < Sys.Date(), 
         RegDate >= as.Date("2019-01-01"))
...
dim_ak_lokalt <- aK_nasjonalt %>% 
  dplyr::filter(.data$Sykehusnavn == params$hospitalName,
...
resultatløsning, Rapporteket, og er basert på data som sist ble oppdatert 
like over midnatt den `r format(Sys.Date(), '%d.%m.%y')`. 
Rapporten distribueres til personer i fagmiljøet som abonnerer på denne
e-post-tjenesten, og er kun ment til internt bruk!
\
...
#Nasjonalt
nstemi_ap_nasjonalt <- aP_nasjonalt[which(aP_nasjonalt$Indikasjon == "NSTEMI" &
                       is.na(aP_nasjonalt$KobletForlopsID) &
                       aP_nasjonalt$innkomstaarsak != "Øvrig" &
                       aP_nasjonalt$ForlopsType2 != "Planlagt" & 
                       !is.na(aP_nasjonalt$AdmissionType) &
                       aP_nasjonalt$TidsDiffSek > 0 &
                       aP_nasjonalt$TidsDiffSek <= 1814400 &
                       !is.na(aP_nasjonalt$TidsDiffSek)
                     ), ]


#Lokalt
nstemiAPTot <- aP_nasjonalt[which(
  aP_nasjonalt$Sykehusnavn == params$hospitalName & 
    aP_nasjonalt$Indikasjon == "NSTEMI" &
    is.na(aP_nasjonalt$KobletForlopsID) &
    aP_nasjonalt$innkomstaarsak != "Øvrig" &
    aP_nasjonalt$ForlopsType2 != "Planlagt" & 
    !is.na(aP_nasjonalt$AdmissionType)
...
### Figurer - Innen 24 timer

__Hittil i år__ 

```{r hittilIAar, results='asis', warning=FALSE}
forsteDagIAar <- lubridate::floor_date(Sys.Date(), "year")

nstemi_ap_hittil_iaar <- nstemiAP %>% 
  dplyr::filter(.data$ProsedyreDato >= forsteDagIAar)
...
```{r fig_ventetid24T_iAar_overflyttet, warning=FALSE, fig.width = 8, fig.height = 4, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.cap="Andel forløp per måned med maks 24 timer ventetid fra starten av dette året til siste hele måned. Analysene inkluderer pasienter overflyttet fra annet sykehus. Vannrett rød stiplet linje markerer forventet måloppnåelse. Måneder med mindre enn 5 totale antall komplette data vises ikke i diagrammet."}
...
__Fra 2017 til siste hele år__
...
nstemiAP_fra2017 <- nstemiAP %>% 
  dplyr::filter(
    .data$Year >= 2017 & 
      .data$ProsedyreDato < lubridate::floor_date(as.Date(Sys.Date()), 
                                                  "year"))
...
fig_tabell <- rbind(fig_lokal, fig_nasj) %>% 
  dplyr::select(.data$Sykehus, 
                .data$Year, 
                .data$name, 
                .data$value, 
...
stentVH_sS_nasjonalt <- sS_nasjonalt %>% 
  ...
  dplyr::select(.data$ForlopsID, .data$Sykehusnavn) 
...
medisinerSS_nasjonalt <- sS_nasjonalt %>%
  ...
  dplyr::ungroup() %>% 
  dplyr::select(.data$Sykehusnavn, .data$ForlopsID, .data$antStent) %>% 
  dplyr::distinct()
...
medisinerAP_nasjonalt <- aP_nasjonalt %>% 
  ...
  dplyr::group_by(.data$Sykehusnavn, .data$primFID) %>% 
...
# Finner kvartalene
  forsteDatoYearAK <- as.Date(paste0(lubridate::year(Sys.Date())-1, "-01-01"))
  sisteDatoKvartalAK <- lubridate::floor_date(as.Date(Sys.Date()), "quarter")-1
  KvartalDatoAK <- seq(forsteDatoYearAK, sisteDatoKvartalAK, by = "quarter")
  
  timeTableQuarterAK <- as.data.frame(KvartalDatoAK) %>%
    dplyr::mutate(Kvartal = paste0(lubridate::year(x = .data$KvartalDatoAK),
                                   " Q",
                                   lubridate::quarter(x = .data$KvartalDatoAK, 
                                        with_year = FALSE))) %>%
    dplyr::select(.data$Kvartal)
...
```{r TabNPacemakerbehovAK, results='asis', warning=FALSE}
...
cap <- "Antall og andel pasienter som har fått implantert permanent pacemaker under sykehusoppholdet etter kateterbasert innsetting av aortaklaff. Kun observasjoner der prosedyreskjemaet er ferdigstilt er tatt med. Kvartaler som vises er fra siste hele år t.o.m. siste hele kvartal fra nåværende år. Hvis andel ferdigstilte prosedyreskjemaer er mindre enn 50\\%, vises ikke prosenten for andel implanterte permanente pacemaker."
