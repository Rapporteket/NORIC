...
\begin{frame}[fragile]
  \frametitle{Lab}
  \framesubtitle{ \Sexpr{ paste0( range( AP$ProsedyreDato ) , collapse = ' -- ' )}}
  
<<FigLabKompl,fig.cap="Prosentandel komplikasjoner på laboratoriet",echo=FALSE,fig.pos='!h',fig.width=10,fig.height=5,out.width="\\textwidth">>=
...
TabNLabKompl <- unlist(lapply( lab.kompl , function(X) sum(AP[,X] == "Ja")))

names(TabNLabKompl) <- c(
    "Alle komplikasjoner" ,
...
TabRLabKompl <- 100 * TabNLabKompl / dim(AP)[1] 

i.labkomp <- order(TabRLabKompl)

TabNLabKompl <- TabNLabKompl[i.labkomp]
TabRLabKompl <- TabRLabKompl[i.labkomp]
...
<<FigAvdKompl,fig.cap="Prosentandel komplikasjoner på avdelingen",echo=FALSE,fig.pos='!h',fig.width=10,fig.height=5,out.width="\\textwidth">>=
...
TabNAvdKompl <- unlist( lapply( avd.kompl , function(X) sum(AP[,X] == "Ja" )))

names(TabNAvdKompl) <- c(
    "Alle komplikasjoner" ,
...
TabRAvdKompl <- 100 * TabNAvdKompl / dim(AP)[1] 

i.avdkomp <- order(TabRAvdKompl)

TabNAvdKompl <- TabNAvdKompl[i.avdkomp]
TabRAvdKompl <- TabRAvdKompl[i.avdkomp]
