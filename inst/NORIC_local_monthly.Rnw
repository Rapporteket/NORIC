...
\section{Komplikasjon}


\begin{frame}[fragile]
  \frametitle{Komplikasjon}
...
LabKomplikasjoner$Levels <- lapply( LabKomplikasjoner$VarName , function(X) paste0( sort( unique( AP[,X] )) , collapse = "," ))

LabKomplikasjoner$N <- unlist(
    lapply(
...
LabKomplikasjoner$R <- round(
    100 * LabKomplikasjoner$N / dim(AP)[1] ,
    digits = 1 )

i.labkomp <- order(LabKomplikasjoner$R)

LabKomplikasjoner <- LabKomplikasjoner[i.labkomp,]

print( LabKomplikasjoner )
...
<<FigLabKompl,eval=FALSE,fig.cap="Prosentandel komplikasjoner på laboratoriet",echo=FALSE,fig.pos='!h',fig.width=10,fig.height=5,out.width="\\textwidth">>=

pal <- RColorBrewer::brewer.pal(4,"Blues")
palk <- c(
    rep(
        x = pal[3] ,
        times = length(TabRLabKompl)-1) ,
    pal[4])
...
AvdKomplikasjoner$Levels <- unlist( lapply( AvdKomplikasjoner$VarName , function(X) paste0( sort( unique( AP[,X] )) , collapse = "," )))

AvdKomplikasjoner$N <- unlist(
    lapply(
...
AvdKomplikasjoner$R <- round(
    100 * AvdKomplikasjoner$N / dim(AP)[1] ,
    digits = 1 )

i.R <- order(AvdKomplikasjoner$R)

AvdKomplikasjoner <- AvdKomplikasjoner[i.R,]

print( AvdKomplikasjoner )
...
<<FigAvdKompl,eval=FALSE,fig.cap="Prosentandel komplikasjoner på avdelingen",echo=FALSE,fig.pos='!h',fig.width=10,fig.height=5,out.width="\\textwidth">>=

pal <- RColorBrewer::brewer.pal(4,"Blues")
palk <- c(
    rep(
        x = pal[3] ,
        times = length(TabRAvdKompl)-1) ,
    pal[4])
