...
I de fire punktene under ønsker vi at at sykehusene sjekker i journalsystemene
...
## Avdød 1-2 dager etter utskrivelse
Følgende pasienter har _AvdodDato_ like etter _Utskrivningsdato_, men 
er verken registrert _død på lab_, _død på avdeling_ eller
_død ved utskrivelse_. Vi ber sykehusene om å sjekke disse registreringene. 
Variabelen _OppholdsID_ grupperer primær- og sekundærforløp som henger sammen,
der det er tilfelle.
...
dod_innen2dg_utskr <- aP %>%  
  dplyr::filter(.data$dod_opphold == "Nei", 
                !is.na(.data$Utskrivningsdato), 
                .data$dager_utskr_dod %in% 1:2, 
                ! .data$Indikasjon %in% "Donorutredning") %>%
  dplyr::select(.data$OppholdsID) %>%
  dplyr::distinct() %>% 
  dplyr::pull()
...
aP %>% 
  dplyr::filter(.data$OppholdsID %in% dod_innen2dg_utskr) %>% 
  dplyr::select(.data$ForlopsID, 
                .data$OppholdsID,
                .data$Regtype,
                .data$ProsedyreDato,
...
<!-- # Donorutredning, ikke registrert død i NORIC -->
...
<!-- vil også bli listet opp her, vi ber dere se bort fra disse.  -->
...
<!-- aP %>%  -->
<!--   dplyr::filter(.data$OppholdsID %in% donor_levende) %>%  -->
<!--   dplyr::select(.data$ForlopsID,  -->
<!--                 .data$OppholdsID, -->
<!--                 .data$Regtype, -->
<!--                 .data$ProsedyreDato, -->
...
avdod_lab_avd <- aP %>%  
  ...
  filter(.data$AvdKompDod_opphold == "Ja",
...
registreringene. Forløp med _Indikasjon_ "Donorutredning” er fjernet. 
Variabelen _OppholdsID_ grupperer primær- og sekundærforløp
som henger sammen, der det er tilfelle.
...
registrering av død, også for pasienter der sykehusene ikke ønsker å
knytte dødsfallet til komplikasjoner på lab eller komplikasjoner på avdeling. 
