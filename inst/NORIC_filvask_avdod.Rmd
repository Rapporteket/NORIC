...
aP %>% 
  dplyr::filter(OppholdsID %in% avdod_lab_avd) %>% 
  dplyr::select(.data$ForlopsID,
                .data$OppholdsID,
                .data$Regtype, 
                .data$ProsedyreDato,
...
Variabelen _OppholdsID_ grupperer primær- og sekundær-forløp
...
dod_uten_utskr <- aP %>%  
  ...
  dplyr::filter(!(AvdKompDod_opphold == "Ja" & LabKompDod_opphold == "Ja"), 
                Regtype == "Primær",
...
registrering av død, også i de forløpene der pasienten er død før utskrivelse 
men  der sykehusene ikke ønsker å knytte dette til komplikasjoner på lab eller
komplikasjoner på avdeling. 
For å kartlegge dette behovet er vi interessert i å vite hvorfor disse 
forløpene er registrert uten _død på lab_ eller _død på avdeling_.  
Vi ber derfor sykehusene om å sjekke disse
registreringne i journal-systemene og om at dere sender oss en liste med
forløpsID og grunn for at pasienten verken er registrertom som avdød på lab 
eller på avdeling på e-post til noric@helse.bergen.no. 
Dere må gjerne komplettere 
_død på lab_ eller _død på avdeling_ i de tilfellene dette manglet uten grunn.
...
ikke_avd_lab <- aP %>%  
  ...
  dplyr::filter(AvdKompDod_opphold == "Nei", 
...
aP %>% 
  dplyr::filter(OppholdsID %in% ikke_avd_lab) %>% 
  dplyr::select(.data$ForlopsID,
                .data$OppholdsID,
                .data$Regtype,
                .data$ProsedyreDato,
