...
AP %<>%
  ...
  dplyr::mutate(
    Nstents = ifelse(is.na(.data$antall_stent),
                     0,
                     .data$antall_stent),
    OpptellingNstents = ifelse(.data$Nstents < 4,
                               .data$Nstents,
                               "\u2265 4"))
...
PCImedStent %<>% mutate(
  OpptellingNstents = factor(OpptellingNstents, 
                             levels = c("1", "2", "3", "\u2265 4"))
) 
...
### Kompletthet komplikasjonsskjemaer
...
cap <- paste0("Antall komplikasjonsskjemaer etter måned for ", params$hospitalName,
...
Antall stenter per prosedyre for `r params$hospitalName` i perioden `r paste0(rangePCImdStentProsDato, collapse = " til ")`. Antall prosedyrer med minst 1 stent er `r sum(PCImedStent$Nstents>0)` i denne perioden.

<!-- ```{r BarplotNstentPerProsedyre, fig.width = 10, fig.height = 4.5, fig.align = "center", fig.pos = "H", out.width = "\\textwidth", fig.cap = paste0("Fordeling per måned for antall stenter per prosedyre der stent er satt inn for ", params$hospitalName, " i perioden ", rangePCImdStentProsDato[1], " til ", rangePCImdStentProsDato[2], ".")} -->

<!-- pal <- colPrimary[c(1,2,3,5)] -->

<!-- opptellingNstentsMaaned <- PCImedStent %>% -->
<!--   dplyr::count(.data$maaned, -->
<!--                .data$OpptellingNstents) %>% -->
<!--   dplyr::group_by(.data$maaned, .data$OpptellingNstents) -->

<!-- OpptellingNstents_levels <- c("\u2265 4", -->
<!--                         "3", -->
<!--                         "2", -->
<!--                         "1") -->

<!-- opptellingNstentsMaaned %>% ggplot2::ggplot( -->
<!--   ggplot2::aes(x = .data$maaned, -->
<!--                y = .data$n, -->
<!--                fill = factor(.data$OpptellingNstents, -->
<!--                              levels = OpptellingNstents_levels))) + -->
<!--   ggplot2::geom_bar(position = "stack", stat = "identity") + -->
<!--   ggplot2::scale_fill_manual(values = pal) + -->
<!--   ggplot2::theme_minimal() + -->
<!--   ggplot2::xlab("Måned") + -->
...
<!-- ```{r TabNstentPerProsedyre, echo = FALSE, results = "asis"} -->
<!-- cap <- paste0("Antall stenter per prosedyre og prosentvis fordeling ", -->
<!--               "per måned for ", params$hospitalName, -->
<!--               " i perioden ", rangeProsDato[1], " til ", -->
<!--               rangeProsDato[2], ".") -->
<!-- xtabs(formula = ~ maaned + OpptellingNstents, data = PCImedStent) %>% -->
<!--   noric::prettyTab(., add_totals = TRUE) %>% -->
<!--   noric::mst(., type = params$tableFormat, cap = cap, digs = 0, -->
<!--              label = "TabNstent") -->
