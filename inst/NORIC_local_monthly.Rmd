...
Antall komplikasjoner oppstått i tilknytning til selve prosedyren mens 
...
Figuren viser sammenlagt antall komplikasjoner de `r showN` siste månedene, 
tabellene viser antall komplikasjoner per måned. 


<br/> <br/>

```{r FigLabKompl, fig.cap = paste0("Antall komplikasjoner oppstått i tilknytning til selve prosedyren mens pasienten fortsatt ", "er på hjertekateteriseringslaboratoriet. ", "Sammenlagt i perioden", rangeProsDato, ". ","Hvis det er registrert flere komplikasjoner på samme ", "pasient er disse talt opp hver for seg. ", "Merk ulik farge for alvorlige og øvrige komplikasjoner.") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 7, fig.align = "center", out.width = "\\textwidth"}
...
synkende_rekkefolge_lab %>%
  ...
  ggplot2::geom_bar(
    ...
    width = 0.85,
    fill = ifelse(
      test = synkende_rekkefolge_lab$gradering == "Alvorlige komplikasjoner på lab",
      yes = colPrimary[4],
      no = colPrimary[6])
  ) +
...
```{r TabLabKompAlvorlig,results='asis'}
cap <- paste0("Antall alvorlige komplikasjoner på kat.lab., ",
              "per måned, i ", 
              "perioden fra ", rangeProsDato,  ".", 
              "")

labKompAlvorlig <- timeTableMonth %>% 
  dplyr::left_join(.,
                   AP %>% 
  ...
  dplyr::left_join(.,
    ...
    dplyr::left_join(.,
                   AP %>% 
                     ...
                     dplyr::count(maaned, .drop = FALSE) %>% 
                     dplyr::rename("Neurologisk komplikasjon" = n), 
                   by = "maaned") %>% 
...
labKompAlvorlig[labKompAlvorlig == 0] <-  " - "
labKompAlvorlig[is.na(labKompAlvorlig)] <- " - "
  
  
  

align <- c("r", rep("c", dim(labKompAlvorlig)[2] - 1))
col_nams <- c("", names(labKompAlvorlig[-1]))

if (params$tableFormat == "html") {
  knitr::kable(x = labKompAlvorlig, 
               caption = cap,
...
```{r TabLabKomplOvrig,results='asis'}
cap <- paste0("Antall øvrige komplikasjoner på kat.lab., ",
              "per måned, i ", 
              "perioden fra ", rangeProsDato,  ".", 
              "")

labKompOvrig <- timeTableMonth %>% 
  dplyr::left_join(.,
                   AP %>% 
                     dplyr::filter(LabKompAllergiskLettModerat %in% "Ja") %>% 
                     dplyr::count(maaned, .drop = FALSE) %>% 
                     dplyr::rename("Allergisk reaksjon lett/moderat" = n), 
                   by = "maaned") %>% 
  dplyr::left_join(.,
                   AP %>% 
    ...
    dplyr::left_join(.,
                   AP %>% 
                     ...
                     dplyr::count(maaned, .drop = FALSE) %>% 
                     dplyr::rename("Vedvarende sidegrensokklusjon" = n), 
                   by = "maaned") %>% 
  janitor::adorn_totals("row", name = "Totalt") 

labKompOvrig[labKompOvrig == 0] <-  " - "
labKompOvrig[is.na(labKompOvrig)] <- " - "
  
  
  

align <- c("r", rep("c", dim(labKompOvrig)[2] - 1))
col_nams <- c("", names(labKompOvrig[-1]))

if (params$tableFormat == "html") {
  knitr::kable(x = labKompOvrig, 
               caption = cap,
               align = align, col.names = col_nams) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped",
                                                    "hover",
...
### Komplikasjoner på avdelingen for `r params$hospitalName` i perioden `r rangeProsDato`
...
```{r FigAvdKompl, fig.cap = paste0("Antall forløp med komplikasjoner på avdelingen registrert i NORIC ved ", params$hospitalName, " i perioden ", rangeProsDato, "."), echo=FALSE, fig.pos = "H", fig.width = 10, fig.height = 5, fig.align = "center", out.width = "\\textwidth"}
