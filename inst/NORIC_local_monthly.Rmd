...
mst <- function(tab, type, cap, digs, fs = 10) {
  if (type == "latex") {
    k <- kable(tab, caption = cap, digits = digs, booktabs = TRUE) %>% 
      kable_styling(latex_options = c("hold_position"), font_size = fs)
  }
  
  if (type == "html") {
    k <- kable(tab, caption = cap, digits = digs) %>% 
