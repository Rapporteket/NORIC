...
TabNInervProsedyreType <- TabNProsedyreTypeMaaned %>% 
  dplyr::select(maaned, "Angio + PCI", "PCI") %>% 
...
TabNSsIntervensjonstype <- timeTableMonth %>% 
  left_join(., TabDataSsIntervensjonstype, by = c("maaned")) %>% 
  right_join(., TabNInervProsedyreType, by = c("maaned")) %>% 
  dplyr::rename(" " = "maaned")

TabNSsIntervensjonstype[is.na(TabNSsIntervensjonstype)] <- 0

align <- c("l", rep("r", dim(TabNSsIntervensjonstype)[2] - 1))
