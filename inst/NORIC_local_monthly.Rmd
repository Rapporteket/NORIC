...
xtabs(formula = ~ Month + Hastegrad, data = AP) %>% 
  100 * prop.table(., margin = 1) %>% 
  kable(., caption = "Prosentandel prosedyrer etter hastegrad og måned",
        digits = 1) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = F)
# xtable::print.xtable(
...
xtabs(formula = ~ Month + ProsedyreType, data = AP) %>% 
  100 * prop.table(., margin = 1) %>% 
  kable(., caption = "Prosentandel prosedyrer etter prosedyretype og måned",
        digits = 1) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = F)
# xtable::print.xtable(
...
```{r FigNIndikasjon,fig.cap="Antall prosedyrer etter indikasjon og måned",fig.width=8,fig.height=6,out.width="\\textwidth"}
...
kable(TabNAndreProsedyrer) %>% 
  ...
  column_spec(column = 1:(dim(TabNAndreProsedyrer)[2]+1), width = "4em")
# kableExtra::row_spec(
