...
AP %<>%
  ...
  dplyr::mutate(
    Nstents = ifelse(is.na(antall_stent),
                     0,
                     antall_stent))
...
fjorårets registreringer på invasiv koronar angiografi og PCI,
...
```{r FigNProsedyreType, fig.cap =  paste0("Antall koronare angiografier og/eller PCI registrert i NORIC per måned ved ", params$hospitalName, " i perioden ", rangeProsDato ,  "."), fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth", warning=FALSE}
...
prosedyreTypeMaaned %>% ggplot2::ggplot(
  ggplot2::aes(x = maaned,
               y = n,
               fill = factor(ProsedyreType,
                             levels = prosedyreType_levels))
) +
  ggplot2::geom_bar(position = "stack", stat = "identity") +
  ggplot2::facet_grid(~ aar, scales = "free_x", space = "free_x") +
...
## Hastegrad
```{r FigNHastegrad, fig.cap = paste0("Antall prosedyrer registrert i NORIC per måned og hastegrad ved ", params$hospitalName, " i perioden ", rangeProsDato, "."), fig.width =8 , fig.height = 5, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", warning=FALSE}
...
hastegradMaaned <- AP %>%
  dplyr::count(maaned,
               Hastegrad,
               aar) %>%
  dplyr::group_by(maaned, Hastegrad, aar)
...
```{r FigNIndikasjon, fig.cap = paste0("Antall prosedyrer registrert i NORIC per måned og indikasjon ved ", params$hospitalName, " i perioden ", rangeProsDato, "."), fig.width = 8, fig.height = 5, fig.pos = "H", fig.align = "center", out.width="\\textwidth", warning=FALSE}
...
Indikasjon2Maaned <- AP %>%
  dplyr::count(maaned,
               Indikasjon2,
               aar) %>%
  dplyr::group_by(maaned, Indikasjon2, aar)
...
cap <- paste0("Antall prosedyrer registrert i NORIC der det er utført ",
...
cap <- paste0("Antall tilleggsprosedyrer registrert i NORIC der det er ",
...
```{r FigLabKompl, fig.cap = paste0("Antall komplikasjoner oppstått i tilknytning til selve prosedyren mens pasienten fortsatt ", "er på hjertekateteriseringslaboratoriet. ", "Sammenlagt i perioden fra ", rangeProsDato, ". Hvis det er registrert flere komplikasjoner på samme ", "pasient er disse talt opp hver for seg. ") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 10, fig.align = "center", out.width = "\\textwidth", warning=FALSE}
...
insert_prsRel_her <- which(synkende_rekkefolge_lab_fjoraaret$name == "LabKompDod",
                           fig_df_kompLab$aar %in% periode_data$sisteHeleYear)

synkende_rekkefolge_lab_fjoraaret %<>%
  dplyr::add_row(
    fig_df_kompLab %>% dplyr::filter(
      name == "LabKompProsedyrerelatertDod",
      fig_df_kompLab$aar %in% periode_data$sisteHeleYear),
...
# REKKEFØLGE:
rekkeflg_komp_lab <- synkende_rekkefolge_lab_fjoraaret %>%
  dplyr::pull(type_komplikasjon) %>% 
  rev()


# PARAMETRE FOR FIGUR: 
endelig_lab <-  fig_df_kompLab %>%
  dplyr::mutate(type_komplikasjon = factor(type_komplikasjon, 
                                           levels = rekkeflg_komp_lab)) %>% 
...
# PLOT
ggplot2::ggplot(data = endelig_lab) +
  ...
  ggplot2::scale_fill_manual(values = colPrimary[c(5, 3)]) +
  ggplot2::scale_alpha_manual(values=c(1, 0.8)) +
...
```{r FigAvdKompl2, fig.cap = paste0("Antall komplikasjoner oppstått på avdelingen.", "Sammenlagt i perioden fra ", rangeProsDato, ". Hvis det er registrert flere komplikasjoner på samme ", "pasient er disse talt opp hver for seg. ") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 12, fig.align = "center", out.width = "\\textwidth", warning=FALSE}
...
fig_df_kompl_avd <- AP %>%
  ...
  select(name, type_komplikasjon, gradering, Ja, aar) 


# Rekkefølge: Rekkefølge fra fjoråret
...
synkende_rekkefolge_avd_alvorlig <- rbind(
  ...
  fig_df_kompl_avd %>% 
    dplyr::filter(name != c("AvdKompAnnenAlvorlig"), 
                  name != c("AvdKompProsedyrerelatertDod"), 
                  gradering %in% "Alvorlige komplikasjoner på avdeling",
                  fig_df_kompl_avd$aar %in% periode_data$sisteHeleYear) %>% 
    dplyr::arrange(-Ja), 
...
# synkende_rekkefolge_avd_alvorlig %>% knitr::kable(align = "r")
...
rekkeflg_komp_avd <- rbind(endelig_alvorlig,
                           endelig_ovrig) %>%
  dplyr::pull(type_komplikasjon) %>% 
  rev()


endelig_alle <- fig_df_kompl_avd %>% 
  dplyr::mutate(type_komplikasjon = factor(type_komplikasjon, 
                                           levels = rekkeflg_komp_avd)) %>% 
  dplyr::arrange(type_komplikasjon) %>% 
  dplyr::group_by(aar) %>% 
  dplyr::mutate(
    Sortering = 1:n(),
...
# PLOT
ggplot2::ggplot(data = endelig_alle) +
  
  ggplot2::geom_bar(
    mapping = ggplot2::aes(x = reorder(type_komplikasjon, Sortering),
                           y = Ja, 
                           fill = fig_df_kompl_avd$aar,
                           alpha = alpha_soyler),
    stat="identity",
    width = 0.8,
    position = ggplot2::position_dodge2(padding=0.1)
  ...
  ggplot2::geom_text(
    mapping = ggplot2::aes(label = Ja,
                           x = reorder(type_komplikasjon,
                                       Sortering),
                           y = Ja,
                           group = fig_df_kompl_avd$aar,
                           color = fig_df_kompl_avd$aar),
    vjust = 0.5,
    hjust = -0.15,
    size = 5,
    position = ggplot2::position_dodge(width = 0.7)
...
TabHovedKomplett <- rbind(
  ...
  mutate(Andel = (round(Ferdigstilt/Totalt, 2) *100))
...
```{r FigNstentPerProsedyre, fig.width = 7, fig.height = 4.5, fig.align = "center", fig.pos = "H", out.width = "\\textwidth", fig.cap = paste0("Antall stenter per prosedyre for ", params$hospitalName, " i perioden ", rangeProsDato, "."), warning=FALSE}
