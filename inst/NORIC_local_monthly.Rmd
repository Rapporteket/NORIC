---
title: "`r paste('Norsk register for invasiv kardiologi (NORIC)', 'MÃ¥nedsrapport Angio/PCI for ', params$hospitalName ) `"
...
xtabs(formula = ~ Month + Hastegrad, data = AP) %>% 
  prettyTab(., add_totals=TRUE) %>% 
  mst(., type = params$tableFormat, cap = cap, digs = 0, align = rep("r", 5))
```
...
xtabs(formula = ~ Month + ProsedyreType, data = AP) %>% 
  prettyTab(., add_totals=TRUE) %>% 
  mst(., type = params$tableFormat, cap = cap, digs = 0)
```
...
xtabs(formula = ~ Month + Indikasjon2, data = AP) %>% 
  prettyTab(., add_totals = TRUE) %>%
  mst(., type = params$tableFormat, cap = cap, digs = 0, align = rep("r", 8),
      lsd = TRUE)
```
...
mst(TabTillegg[,c("FFR", "IFR", "Doppler", "OCT", "IVUS", "NIRS", "N")],
...
cap <- "Antall (prosent) tilleggsprosedyrer per måned"
mst(TabTillegg[,c("Høyre kat.", "Perikardiocentese", "N")],
    type = params$tableFormat,
    cap = cap,
    digs = 0,
    align = rep("r", 4))
...
cap <- "Antall (prosent) tilleggsprosedyrer per måned"
mst(TabTillegg[, c("PM", "IABP", "Impella", "Trombectomy", "ECMO", "Annen",
                   "Ingen", "N")],
    type = params$tableFormat,
    cap = cap,
    digs = 0,
...
### Komplikasjoner på lab i perioden `r paste0( range( AP$ProsedyreDato ) , collapse = ' -- ' )`
...
### Komplikasjoner på avdelingen i perioden `r paste0( range( AP$ProsedyreDato ) , collapse = ' -- ' )`
...
mst(df,
    type = params$tableFormat,
    cap = "Komplikasjoner på avdelingen.",
    digs = 2,
    align = c("l","r","r"))
...
cap <- "Antall ferdigstilte hovedskjemaer per måned"
...
cap <- "Antall ferdigstilte utrskrivelsesskjemaer etter måned"
mst(tab = TABkompletthet, type = params$tableFormat, cap = cap,
    digs = c(0,0,0,1), align = rep("r", 4))
```
