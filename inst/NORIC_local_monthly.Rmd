---
title: "`r paste('Norsk register for invasiv kardiologi (NORIC)', 'Månedsrapport Angio/PCI og stentbruk for ', params$hospitalName) `"
author: ""
...
abstract: '`r paste0("Denne rapporten er generert ved hjelp av resultatløsningen _Rapporteket_. Rapporten er kun ment til internt bruk! Rapporten inneholder tabeller og figurer med resultater basert på forløp registrert ved ", params$hospitalName, ". Opptellinger og statistikker er gjort per forløp/prosedyre, og ikke per individ. De fleste tabellene og figurene presenteres med data fra de siste 14 månedene." , "\\newline ", "\\newline ", "Eventuelle spørsmål knyttet til rapporten kan rettes til [noric@helse-bergen.no](mailto:noric@helse-bergen.no)") `'
---
...
# crude fix for Feiring and Rikshospitalet which have meaningless test data
# before 2015
if (AP$Sykehusnavn[1] %in% c("Feiring", "Rikshospitalet")) {
  AP <- AP[which(AP$Year >= 2015), ]
}
...
<!--   p(HTML("<h1>Månedsrapport: Angio/PCI og stentbruk</h1><br/>"), -->
<!--     "Denne rapporten er generert ved hjelp av resultatløsningen ", -->
<!--     HTML("<i>Rapporteket</i>"), -->
<!--   ". Rapporten er kun ment til internt bruk! Rapporten inneholder tabeller og figurer med resultater basert på forløp registrert ved ", -->
<!--     params$hospitalName, -->
...
# Månedsrapport: Angio/PCI og stentbruk {-}
Denne rapporten er generert ved hjelp av resultatløsningen *Rapporteket* og er kun ment til internt bruk! 
<br/> <br/>
Rapporten inneholder tabeller og figurer med resultater basert på forløp registrert ved `r params$hospitalName`. Opptellinger og statistikker er gjort per forløp/prosedyre, og ikke per individ. De fleste tabellene og figurene presenteres med data fra de siste 14 månedene.
<br/><br/>
...
## Hastegrad
```{r FigNHastegrad, fig.cap = paste0("Antall prosedyrer etter hastegrad og måned for ", params$hospitalName, " i perioden ", rangeProsDato[1], " til ", rangeProsDato[2], "."), fig.width = 10, fig.height = 4.5, fig.pos = "H", fig.align = "center", out.width = "\\textwidth"}
...
barplot(
  xtabs(formula = ~ Hastegrad + maaned, data = AP),
  las = 1,
  xlab = "Måned",
  ylab = "Antall",
  col = pal,
...
legend(
  ...
  horiz = FALSE,
  fill = pal[3:1],
  legend = sort(unique(AP$Hastegrad), decreasing = TRUE),
  cex = 1)
```

```{r TabNHastegrad, results='asis', eval=TRUE}
cap <- paste0("Antall og prosentvis fordeling av registrerte prosedyrer etter",
              " hastegrad og måned for ", params$hospitalName,
...
## Prosedyretype
```{r FigNProsedyreType, fig.cap =  paste0("Antall prosedyrer etter type og måned for ", params$hospitalName, " i perioden ", rangeProsDato[1], " til ", rangeProsDato[2], "."), fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth"}
pal <- colPrimary[c(1, 3, 5)]
...
barplot(
  xtabs(formula = ~ ProsedyreType + maaned, data = AP),
  las = 1,
  xlab = "Måned",
  ylab = "Antall",
  col = pal,
...
cap <- paste0("Antall og prosentvis fordeling av prosedyrer etter",
              " prosedyretype og måned for ", params$hospitalName,
              " i perioden ", rangeProsDato[1], " til ",
              rangeProsDato[2], ".")
...
## Indikasjon
```{r FigNIndikasjon, fig.cap = paste0("Antall prosedyrer etter indikasjon og måned for ", params$hospitalName, " i perioden ", rangeProsDato[1], " til ", rangeProsDato[2], "."), fig.width = 12, fig.height = 6, fig.pos = "H", fig.align = "center", out.width="\\textwidth"}
...
barplot(
  xtabs(formula = ~ Indikasjon2 + maaned, data = AP),
  las = 1,
  xlab = "Måned",
  ylab = "Antall",
  col = colPrimary[6:1],
...
cap <- paste0("Antall og prosentvis fordeling av prosedyrer etter ",
              "indikasjon og måned for ", params$hospitalName, " i perioden ",
...
cap <- paste0("Antall (prosent) tilleggsprosedyrer per måned for ",
...
cap <- paste0("Antall andre prosedyrer (utført uten direkte tilknytning",
             " til Angio/PCI)  etter prosedyretype og måned for ",
             params$hospitalName, " i perioden ",
             rangeProsDato[1], " til ", rangeProsDato[2], ".")
TabNAndreProsedyrer <- addmargins(
 xtabs(formula = ~ maaned + AnnenProsType,
       data = AnP)
)
...
## Komplikasjoner
### Komplikasjoner på lab i perioden `r paste0(rangeProsDato, collapse = ' til ' )`
...
```{r FigLabKompl, fig.cap = paste0("Prosentandel komplikasjoner på laboratoriet for ", params$hospitalName, " i perioden ", rangeProsDato[1], " til ", rangeProsDato[2], "."), echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 5, fig.align = "center", out.width = "\\textwidth"}
...
lk <- LabKomplikasjoner[, 4]
names(lk) <- LabKomplikasjoner[, 2]
...
### Komplikasjoner på avdelingen i perioden `r paste0(rangeProsDato, collapse = ' til ' )`
...
```{r FigAvdKompl, fig.cap = paste0("Prosentandel komplikasjoner på avdelingen for ", params$hospitalName, " i perioden ", rangeProsDato[1], " til ", rangeProsDato[2], "."), echo=FALSE, fig.pos = "H", fig.width = 10, fig.height = 5, fig.align = "center", out.width = "\\textwidth"}
...
ak <- AvdKomplikasjoner[, 4]
names(ak) <- AvdKomplikasjoner[, 2]
...
cap <- paste0("Antall hoved-registreringsskjemaer etter måned for ",
             params$hospitalName, " i perioden ", rangeProsDato[1],
             " til ", rangeProsDato[2], ".")
...
cap <- paste0("Antall utskrivelsesskjemaer etter måned for ", params$hospitalName,
...
cap <- paste0("Antall (prosent) behandlede segmenter etter prosedyretype ",
              "og måned for ", params$hospitalName, " i perioden ",
              rangeProsDato[1], " til ", rangeProsDato[2], ".")
...
Antall stenter per prosedyre  for `r params$hospitalName` i perioden `r paste0(rangePCImdStentProsDato, collapse = " til ")`. Antall prosedyrer med minst 1 stent er `r sum(PCImedStent$Nstents>0)`.

```{r BarplotNstentPerProsedyre, fig.width = 12, fig.height = 4.5, fig.align = "center", fig.pos = "H", out.width = "\\textwidth", fig.cap = paste0("Antall stenter per prosedyre for ", params$hospitalName, " i perioden ", rangePCImdStentProsDato[1], " til ", rangePCImdStentProsDato[2], ".")}
...
cap <- paste0("Antall stenter etter merke og måned for ", params$hospitalName,
              " i perioden ", rangePCImdStentProsDato[1], " til ",
              rangePCImdStentProsDato[2], ".")
tab_stent <- addmargins(xtabs(formula = ~ maaned + Stentnavn,
                             data = SSpci,
