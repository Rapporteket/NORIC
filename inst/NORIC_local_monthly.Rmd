...
## Hastegrad
```{r FigNHastegrad,fig.cap="Antall prosedyrer etter hastegrad og måned",fig.width=10,fig.height=4.5,fig.pos='H',out.width="\\textwidth"}
...
## Prosedyretype
```{r FigNProsedyreType,fig.cap="Antall prosedyrer etter type og måned",fig.width=8,fig.height=5, fig.pos="H", out.width="\\textwidth"}
pal <- colPrimary[c(1,3,5)]
...
## Indikasjon
```{r FigNIndikasjon,fig.cap="Antall prosedyrer etter indikasjon og måned",fig.width=12,fig.height=6, fig.pos="H",out.width="\\textwidth"}
...
# Antall prosedyrer å legge til i hver tabell, slik forstår vi hvordan 
# prosenten er regnet ut
TabTillegg %>% 
  cbind(., AP_HUS %>% 
          count(Month) %>%
          adorn_totals("row") %>%
...
mst(TabTillegg[,c("FFR", "IFR", "Doppler", "OCT", "IVUS", "NIRS", "N")], params$tableFormat, cap, 0, align = rep("r", 8))
```
...
cap <- "Antall (prosent) tilleggsprosedyrer per måned"
mst(TabTillegg[,c("Høyre kat.", "Perikardiocentese", "N")], params$tableFormat, cap = cap, 0, align = rep("r", 4))
```
...
cap <- "Antall (prosent) tilleggsprosedyrer per måned"
mst(TabTillegg[,c("PM", "IABP", "Impella", "Trombectomy", "ECMO", "Annen", "Ingen", "N")],
    params$tableFormat, cap, 0, align = rep("r", 8), lsd = TRUE)
```
