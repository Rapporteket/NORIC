...
```{r GetData, include=TRUE, cache=FALSE}
...
```{r SOPrepare,include=TRUE, eval=TRUE}
...
```{r APprepare,include=TRUE, eval=TRUE}
...
```{r Daterecodes,include=TRUE, eval=TRUE}
...
```{r subsetting,include=TRUE, eval=TRUE}
...
tab <- xtabs(formula = ~ Month + Indikasjon2, data = AP) %>% 
  prop.table(., margin = 1) * 100
mst(tab, params$tableFormat, cap, 1, align = rep("r", 7))
```
...
`r if(evaluate_AnP == FALSE){"Dette lokale registeret har ingen registrerte forløp fra Andre Prosedyrer-modulen i nåværende datagrunnlag."}`
