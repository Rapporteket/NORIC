...
AP %<>% noric::legg_til_trykkmaalinger(., df_ad = AnD) 
...
TabNProsedyreType <- timeTableMonth %>% 
  left_join(., TabDataProsedyreType, by = c("maaned")) %>% 
  dplyr::rename(" " = "maaned")

TabNProsedyreType[is.na(TabNProsedyreType)] <- 0

align <- c("l", rep("r", dim(TabNProsedyreType)[2] - 1))
...
TabNHastegrad <- timeTableMonth %>% 
  left_join(., TabDataHastegrad, by = c("maaned")) %>% 
  dplyr::rename(" " = "maaned")

TabNHastegrad[is.na(TabNHastegrad)] <- 0

align <- c("l", rep("r", dim(TabNHastegrad)[2] - 1))
...
TabNIndikasjon <- timeTableMonth %>% 
  left_join(., TabDataIndikasjon, by = c("maaned")) %>% 
  dplyr::rename(" " = "maaned")

TabNIndikasjon[is.na(TabNIndikasjon)] <- 0
...
TabTillegg <- cbind(
  ...
  PrintTabTilleggCol("Pa"),
  PrintTabTilleggCol("Pd"),
...
### Annen diagnostikk ved direkte tilknytning til Angio/PCI
...
### Tilleggsprosedyrer ved direkte tilknytning til Angio/PCI
...
## Komplikasjoner
### Komplikasjoner på kat.lab. for `r params$hospitalName`  i perioden `r paste0(rangeProsDato, collapse = ' til ' )`

Antall ulike komplikasjoner  oppstått i tilknytning til selve prosedyren mens pasienten fortsatt
er på hjertekateteriseringslaboratoriet.
Datagrunnlaget for komplikasjoner på kat.lab. gjelder for `r AP %>% nrow()` forløp.
...
i.labkomp <- rev(order(LabKomplikasjoner_uPRD$Andel))
...
cap <- paste0(
  "Antall (andel) forløp med komplikasjoner på kat.lab. registrert i NORIC ved ",
  params$hospitalName, " i perioden ",
  rangeProsDato[1], " til ", rangeProsDato[2], ". ",
  "Alle forløp i perioden er med i datagrunnlaget, totalt ", nrow(AP), ".")
...
```{r FigLabKompl, fig.cap = paste0("Antall forløp med komplikasjoner på kat.lab. registrert i NORIC ved ", params$hospitalName, " i perioden ", rangeProsDato[1], " til ", rangeProsDato[2], "."), echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 5, fig.align = "center", out.width = "\\textwidth"}
...
i.labkomp <- order(LabKomplikasjoner_uPRD$Andel)
...
Antall komplikasjoner oppstått på sengepost under sykehusoppholdet etter avsluttet prosedyre. Hvis det
er registrert flere komplikasjoner på samme pasient er disse talt opp hver for seg. 

Dette gjelder altså for `r AP %>% dplyr::filter(.data$Regtype %in% "Primær") %>% nrow()` forløp og er uavhengig av kompletthet.

[***Klikk her***](#komplSkjema) for å se oversikten over antall (andel) ferdigstilte komplikasjonsskjemaer.
...
i.avdKomp <- rev(order(AvdKomplikasjoner_uPRD$Andel))
...
cap <- paste0(
  "Antall (andel) forløp med komplikasjoner på avdelingen registrert i NORIC ",
  "ved ", params$hospitalName, " i perioden ",
  rangeProsDato[1], " til ", rangeProsDato[2], ". ",
  "Antall komplikasjoner gjelder per opphold, totalt ", nrow(AP_avdKomp), ".")
...
```{r FigAvdKompl, fig.cap = paste0("Antall forløp med komplikasjoner på avdelingen registrert i NORIC ved ", params$hospitalName, " i perioden ", rangeProsDato[1], " til ", rangeProsDato[2], "."), echo=FALSE, fig.pos = "H", fig.width = 10, fig.height = 5, fig.align = "center", out.width = "\\textwidth"}
...
i.avdKomp <- order(AvdKomplikasjoner_uPRD$Andel)
...
Opptellingene under gjelder prosedyretypene Angio/PCI og PCI. 

[***Her***](#prosedyretype) er en oversikt over antall prosedyrer per måned for disse to prosedyretypene.
...
TabDataSsProsedyreType <- SS %>% 
  janitor::tabyl(maaned,
                 ProsedyreType2) %>% 
  janitor::adorn_totals("col", name = "Totalt") 

TabNSsProsedyreType <- timeTableMonth %>% 
  left_join(., TabDataSsProsedyreType, by = c("maaned")) %>% 
  dplyr::rename(" " = "maaned")

TabNSsProsedyreType[is.na(TabNSsProsedyreType)] <- 0

align <- c("l", rep("r", dim(TabNSsProsedyreType)[2] - 1))

if (params$tableFormat == "html") {
  knitr::kable(x = TabNSsProsedyreType, caption = cap, align = align) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped",
                                                    "hover",
                                                    "condensed"),
                              full_width = FALSE) %>%
  ...
  knitr::kable(x = TabNSsProsedyreType,
               format = params$tableFormat,
               caption = cap,
               booktabs = TRUE,
               align = "r") %>%
...
Antall stenter per prosedyre ved `r params$hospitalName` registrert i NORIC i perioden `r paste0(rangePCImdStentProsDato[1], " til ", rangePCImdStentProsDato[2])`. Antall prosedyrer med minst 1 stent er `r sum(PCImedStent$Nstents>0)`.
...
align <- c("r", rep("r", dim(tab_stent)[2] - 1))
...
Prosedyrer som er utført uten tilknytning til Angio/PCI. 
...
TabNAndreProsedyrer <- timeTableMonth %>% 
  left_join(., TabDataAndreProsedyrer, by = c("maaned")) %>% 
  dplyr::rename(" " = "maaned")

TabNAndreProsedyrer[is.na(TabNAndreProsedyrer)] <- 0

align <- c("r", rep("r", dim(TabNAndreProsedyrer)[2] - 1))
