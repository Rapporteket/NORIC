  ...
  tidyr::pivot_longer(cols = AvdKompVaskulaer:AvdKompAllergisk) %>% 
   ...
   dplyr::mutate(
    type_komplikasjon = case_when(
      ...
      name %in% "AvdKompNeurologiskKomp" ~ "Neurologisk", 
      ...
      name %in% "AvdKompAllergisk" ~ "Allergisk reaksjon", 
      TRUE ~ NA_character_ ), 
...
endelig_alle %>%
  ...
  ggplot2::geom_bar(
    ...
    fill = colPrimary[4]) +
  #   fill = ifelse(
  #     test = synkende_rekkefolge_avd2$gradering2 == "Alvorlige komplikasjoner på avdeling",
  #     yes = ,
  #     no = colPrimary[6])
  # ) +
