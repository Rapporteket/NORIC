...
<<"knitrSettings", include=FALSE, echo=FALSE, eval=TRUE>>=
...
<<GetSampleData, include=FALSE, eval=TRUE>>=
data(SegmentStent, package="noric")
@

<<GetData,include=FALSE, eval=FALSE>>=

baseName <- "noricStaging"
registryName <- NORICmakeRegistryName(baseName, reshID)
SegmentStent <- NORICloadRegDataStent(registryName)
AngioPCI <- NORICloadRegDataAngioPCI(registryName)
@

<<MassageData, include=FALSE>>=
