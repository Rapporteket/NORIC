...
## add stentbruk to AP
StentN <- as.data.frame(
    with(
        SS[ which(SS$StentType!=""),] ,
        table( ID )) ,
    stringsAsFactors=FALSE )
StentN$ID <- as.numeric(StentN$ID)
names(StentN) <- c("ID","N.stent")
...
AP$N.stent[ which( is.na( AP$N.stent)) ] <- 0
...
\begin{frame}[fragile]
<<NstentNameMonth,echo=FALSE,results='asis'>>=
print(
    xtable::xtable(
        addmargins(
            xtabs(
                formula = ~ Stentnavn2 +YearMonth ,
                data = SS )) ,
...
\begin{frame}[fragile]
<<RstentNameMonth,echo=FALSE,results='asis'>>=
    ...
    xtable::xtable(
        100 * prop.table(
            addmargins(
                ...
                margin = 2 ) ,
            margin = 2) ,
        digits = 1 , caption = "Prosentandel stent etter merke og måned") ,
    booktabs = TRUE ,
    rotate.colnames = TRUE,
    scalebox = 0.95)
