...
if (params$rendered_by_shiny == TRUE)
  shiny::setProgress(0.01)  # set progress to 5%
```
...
if (params$registryName %in% "noric_nasjonal") {
  nyeste_reg <- min(
    (as.Date(Sys.time()) - 1),
...
```{r }
if (params$rendered_by_shiny == TRUE )
  shiny::setProgress(0.05)  # set progress to 10%
```
  ...
  dplyr::mutate(
    ...
    TypeKlaffeprotese2 = factor(
      x = dplyr::case_when(
        ...
        TypeKlaffeprotese %in% c("NAVITOR") ~ "NAVITOR", 
        
        !is.na(TypeKlaffeprotese) ~ "Annet", 
        is.na(TypeKlaffeprotese) ~ NA_character_), 
      levels = c("Edwards", "CoreValve", "NAVITOR", "Annet", NA_character_)), 
...
```{r FigKumulertProdePrep, echo = FALSE}
...
ylim <- c(0, ceiling(max(df_cumplot$max_tota, na.rm = TRUE)/50)*50)

plot_kumul <- ggplot2::ggplot(df_cumplot) +
  ggplot2::geom_line(
...
```{r FigKumulertpdf, include=knitr::is_latex_output(), fig.cap =  paste0("Kumulert antall prosedyrer på aortaklaff registrert i NORIC ved ", params$hospitalName, " for hele fjoråret og hittil i år (t.o.m ", siste_dato_txt , ")."), fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.width = 8, fig.height = 5}
  plot_kumul
```


```{r FigKumulertProdehtml, include=knitr::is_html_output(), fig.cap =  paste0("Kumulert antall prosedyrer på aortaklaff registrert i NORIC ved ", params$hospitalName, " for hele fjoråret og hittil i år (t.o.m ", siste_dato_txt , ")."), fig.align = "center", , fig.width = 8, fig.height = 5}
  plot_kumul
```
...
cap <- paste0("Fordeling av operativ tilgang registrert i NORIC ",
              ...
              " for hele fjoråret og hittil i år (t.o.m ",
              siste_dato_txt, 
              "). Gjelder kun ved prosedyretype TAVI.")

TabOpTilg_fjor <- merge(
  x = timeTableMonth_fjor,
  y = AK %>%  
    filter(Prosedyre %in% c("TAVI")) %>% 
...
if (params$tableFormat == "html") {
  TabOpTilg %>%
    dplyr::select(-aar) %>% rename(" " = "maaned") %>% 
    knitr::kable(caption = cap, align = align) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped",
                                                    "hover",
                                                    "condensed"),
...
if (params$tableFormat == "html") {
  TabTypeKlaff %>% dplyr::select(-aar) %>% rename(" " = "maaned") %>% 
    knitr::kable(caption = cap, align = align) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped",
                                                    "hover",
                                                    "condensed"),
...
if (params$tableFormat == "html") {
  klaffiklaff %>% dplyr::select(-aar) %>% rename(" " = "maaned") %>% 
    knitr::kable(caption = cap, align = align) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped",
                                                    "hover",
                                                    "condensed"),
...
cap <- paste0("Antall prosedyrer der postoperativ EKKO er utført og hvor ", 
              "paravalvular insuffisiens er registrert i NORIC ",
              "ved ", params$hospitalName,
              " for hele fjoråret og hittil i år (t.o.m ",
              siste_dato_txt,
              "). Gjelder kun ved prosedyretype TAVI.")
...
paraInsuff_fjor <- merge(
  x = timeTableMonth_fjor,
  y = AK %>%
    filter(PostUndersokelseForetatt %in% "Ja",
           Prosedyre %in% "TAVI") %>% 
...
if (params$tableFormat == "html") {
  paraInsuff %>% dplyr::select(-aar) %>% rename(" " = "maaned") %>% 
    knitr::kable(caption = cap, align = align) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped",
                                                    "hover",
                                                    "condensed"),
...
# PLOT
plot_komplik_lab <- ggplot2::ggplot(data = fig_df_kompLab2) +
...
```{r FigLabKomplpdf, include=knitr::is_latex_output(), fig.cap = paste0("Antall komplikasjoner oppstått i tilknytning til selve prosedyren mens pasienten fortsatt ", "er på lab. For alle prosedyrer fra fjoråret og hittil i år (t.o.m ", siste_dato_txt,"). Hvis det er registrert flere komplikasjoner på samme pasient er disse talt opp hver for seg.") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 10, fig.align = "center", out.width = "\\textwidth"}

plot_komplik_lab
```

```{r FigLabKomplhtml, include=knitr::is_html_output(), fig.cap = paste0("Antall komplikasjoner oppstått i tilknytning til selve prosedyren mens pasienten fortsatt ", "er på lab. For alle prosedyrer fra fjoråret og hittil i år (t.o.m ", siste_dato_txt,"). Hvis det er registrert flere komplikasjoner på samme pasient er disse talt opp hver for seg.") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 10, fig.align = "center"}
...
if (params$tableFormat == "html") {
  labKompTot %>% dplyr::select(-aar) %>% rename(" " = "maaned") %>% 
    knitr::kable(caption = cap, align = align) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped",
                                                    "hover",
                                                    "condensed"),
...
# PLOT
plot_avd_komp <- ggplot2::ggplot(data = fig_df_kompAvd2) +
...
```{r FigAvdKomplPdf, include=knitr::is_latex_output(), fig.cap = paste0("Antall komplikasjoner på avdeling etter prosedyren. For alle prosedyrer fra fjoråret og hittil i år (t.o.m ", siste_dato_txt, "). Hvis det er registrert flere komplikasjoner på samme ", "pasient er disse talt opp hver for seg. ") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 10, fig.align = "center", out.width = "\\textwidth"}

plot_avd_komp
```

```{r FigAvdKomplHtml, include=knitr::is_html_output(), fig.cap = paste0("Antall komplikasjoner på avdeling etter prosedyren. For alle prosedyrer fra fjoråret og hittil i år (t.o.m ", siste_dato_txt, "). Hvis det er registrert flere komplikasjoner på samme ", "pasient er disse talt opp hver for seg. ") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 10, fig.align = "center"}
...
```{r figVentetidPrep, warning=FALSE}
plot_ventetid_tavi <- AK %>% 
...
```{r figVentetidPdf, include=knitr::is_latex_output(), fig.cap = paste0("Ventetid for planlagte prosedyrer på aortaklaff er beregnet som antall dager mellom dato for screeningbeslutning og prosedyredatoen. Prosedyrene er fra tidsrommet fra hele fjoråret og hittil i år (t.o.m ", siste_dato_txt , ")."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth", warning=FALSE}
plot_ventetid_tavi
```


```{r figVentetidHtml, include=knitr::is_html_output(), fig.cap = paste0("Ventetid for planlagte prosedyrer på aortaklaff er beregnet som antall dager mellom dato for screeningbeslutning og prosedyredatoen. Prosedyrene er fra tidsrommet fra hele fjoråret og hittil i år (t.o.m ", siste_dato_txt , ")."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", warning=FALSE}
plot_ventetid_tavi
```
...
```{r figLiggetidPdf, include=knitr::is_latex_output(), fig.cap = paste0("Liggetid for prosedyrer på aortaklaff er beregnet som antall dager mellom prosedyredatoen og utskrivelsesdasto. Negative liggetider (feilregistrert), eller liggetider over 30 dager er fjernet. Prosedyrene er fra hele fjoråret og hittil i år (t.o.m ", siste_dato_txt ,"). Merk at antall prosedyrer i hver gruppe kan være svært ulikt."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth"}
plot_liggetid
```

```{r figLiggetidHtml, include=knitr::is_html_output(), fig.cap = paste0("Liggetid for prosedyrer på aortaklaff er beregnet som antall dager mellom prosedyredatoen og utskrivelsesdasto. Negative liggetider (feilregistrert), eller liggetider over 30 dager er fjernet. Prosedyrene er fra hele fjoråret og hittil i år (t.o.m ", siste_dato_txt ,"). Merk at antall prosedyrer i hver gruppe kan være svært ulikt."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center"}
plot_liggetid
```
...
fromDate <- max(as.Date(paste0(periode_data$nyesteRegYear - 9, "-01-01"), 
                        format = "%Y-%m-%d"),
...
query <- paste0(
  "SELECT 
   CASE
     WHEN IFNULL(patient.GENDER,0) = 0 THEN 'Ikke angitt'
     WHEN patient.GENDER = 1 THEN 'Mann'
     WHEN patient.GENDER = 2 THEN 'Kvinne'
     ELSE 'Ukjent'
   END AS PasientKjonn,
 ...
 WHERE 
   mce.INTERVENTION_TYPE = 5 AND ", 
 "mce.INTERDAT >= '", fromDate, "' AND
   mce.INTERDAT <= '", toDate, "'" )

if(params$registryName %in% "noric_nasjonal") {
  query <- paste0(query, " AND mce.CENTREID = '", params$reshId, "' ")
}
query <- paste0(query, " ;")

AK_alderKjonn <- rapbase::loadRegData(params$registryName, query) %>%
  noric::utlede_alder(.) %>%
...
```{r figalderkjonnPrep}
plot_alder_kjonn <- AK_alderKjonn %>%
...
```{r figalderkjonnPdf, include=knitr::is_latex_output(), fig.cap = paste0("Pasientens alder ved prosedyren, for kvinner og menn, per i år fra ", fromDate," til ", toDate, "."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth"}
plot_alder_kjonn
```


```{r figalderkjonnHtml, include=knitr::is_html_output(), fig.cap = paste0("Pasientens alder ved prosedyren, for kvinner og menn, per i år fra ", fromDate," til ", toDate, "."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center"}
plot_alder_kjonn
```
