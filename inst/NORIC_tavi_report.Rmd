...
```{r}
if (params$rendered_by_shiny == TRUE && params$tableFormat == "latex")
    shiny::setProgress(0.5)  # set progress to 30%
```
...
```{r figVentetid, fig.cap =cap, fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth"}

cap <- paste0("Ventetid for planlagte prosedyrer på aortaklaff er beregnet som ", 
              "antall dager mellom dato for screeningbeslutning og prosedyredatoen. ", 
              "Boxplotet viser fordelingen av ventetiden. ", 
              "De blå boksene er avgrenset av nedre og øvre kvartil,  ",
              "og inneholder 50% av registreringene.",
              "Medianen er markert av den tykke linjen midt i boksen.", 
              "De vertikale greinene viser minste (største) verdi i datasettet ", 
              "innenfor 1,5 ganger interkvartil bredde under nedre kvartil ", 
              "(over øvre kvartil). ", 
              "Punktene utenfor greinene viser uteliggere, det vil si " ,  
              "forløp med usedvanlig kort eller lang ventetid ", 
              "(ventetid lengre enn 1,5 ganger interkvartil bredde over øvre ", 
              "kvartil eller under nedre kvartil). ", 
              "Merk at registreringer for innneværende år ikke nødvendigvis er 'filvasket'.")
AK %>% 
