...
Datagrunnlaget omfatter alle prosedyrer, uavhengig av pasientens helsetilstand 
eller prosedyrens hastegrad. 
...
```{r FigLabKompl, fig.cap = paste0("Antall komplikasjoner oppstått i tilknytning til selve prosedyren mens pasienten fortsatt ", "er på hjertekateteriseringslaboratoriet. For alle prosedyrer fra fjoråret og hittil i år. Hvis det er registrert flere komplikasjoner på samme pasient er disse talt opp hver for seg.") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 10, fig.align = "center", out.width = "\\textwidth"}
...
if (params$tableFormat == "html") {
  labKompTot %>% dplyr::select(-aar) %>% 
    ...
    kableExtra::row_spec(row=12, hline_after =  TRUE) %>%
    kableExtra::row_spec(row=13, hline_after =  TRUE) %>%
    kableExtra::row_spec(row=dim(labKompTot)[1]-1, hline_after =  TRUE) %>%
    kableExtra::pack_rows(group_label = as.character(periode_data$sisteHeleYear),
                          start_row = 1, 
                          end_row = 13) %>% 
    kableExtra::pack_rows(group_label = as.character(periode_data$nyesteRegYear),
...
samme pasient er disse talt opp hver for seg. 
...
fig_df_kompAvd <- AK %>%
  dplyr::select(
    ...
    AvdKompBlodning_fatal, 
    AvdKompBlodning_minor,
  ...
  dplyr::mutate(
    type_komplikasjon = case_when(
      ...
      name %in% "AvdKompBlodning_minor" ~ "Blødning minor",
      name %in% "AvdKompBlodning_major" ~ "Blødning major",
      name %in% "AvdKompBlodning_fatal" ~ "Blødning livstruende/fatal",
...
rekke_avd <- rbind(
  # Blødning i synkende alv grad
  fig_df_kompAvd %>% 
    filter(name %in% "AvdKompBlodning_fatal",
           aar %in% periode_data$sisteHeleYear), 
  fig_df_kompAvd %>% 
    filter(name %in% "AvdKompBlodning_major",
           aar %in% periode_data$sisteHeleYear), 
  fig_df_kompAvd %>% 
    filter(name %in% "AvdKompBlodning_minor",
           aar %in% periode_data$sisteHeleYear), 
  
  # Resten, rekkefølge fjoråret                    
  fig_df_kompAvd %>% 
...
```{r FigAvdKompl, fig.cap = paste0("Antall komplikasjoner på avdeling etter prosedyren. For alle prosedyrer fra fjoråret og hittil i år.",  " Hvis det er registrert flere komplikasjoner på samme ", "pasient er disse talt opp hver for seg. ") , echo=FALSE, fig.pos="H", fig.width = 10, fig.height = 10, fig.align = "center", out.width = "\\textwidth"}
...
avdKomp_fjor <- merge(
  x = timeTableMonth_fjor,
  y = AK %>%  
    dplyr::filter(AvdKompHjerneslag %in% "Ja") %>% 
    dplyr::count(maaned, .drop = FALSE) %>% 
    dplyr::rename("Hjerneslag" = n), 
  by.x  = "maaned", 
  by.y = "maaned") %>% 
  ...
  merge(x = ., 
        y = AK %>%  
          ...
          dplyr::count(maaned, .drop = FALSE) %>% 
          dplyr::rename("-herav svære sekveler" = n),  
        by.x  = "maaned", 
        by.y = "maaned") %>% 
 merge(x = ., 
        y = AK %>%  
          ...
          dplyr::count(maaned, .drop = FALSE) %>% 
          dplyr::rename("-herav moderate sekveler" = n),  
        by.x  = "maaned", 
        by.y = "maaned") %>% 
   ...
   merge(x = ., 
        y = AK %>%  
          ...
          dplyr::count(maaned, .drop = FALSE) %>% 
          dplyr::rename("-herav lette sekveler" = n),  
        by.x  = "maaned", 
        by.y = "maaned") %>% 
  ...
  merge(x = ., 
        y = AK %>%  
          dplyr::filter(AvdKompBlodning %in% "Ja") %>% 
          dplyr::count(maaned, .drop = FALSE) %>% 
          dplyr::rename("Blødning" = n),  
        by.x  = "maaned", 
        by.y = "maaned") %>% 
  ...
  merge(x = ., 
        y = AK %>%  
          ...
          dplyr::count(maaned, .drop = FALSE) %>% 
          dplyr::rename("-herav fatal el livstruende" = n),  
        by.x  = "maaned", 
        by.y = "maaned") %>% 
  merge(x = ., 
        y = AK %>%  
  ...
  merge(x = ., 
        y = AK %>%  
          dplyr::filter(AvdKompInfeksjon_janei %in% "Ja") %>% 
          dplyr::count(maaned, .drop = FALSE) %>% 
          dplyr::rename("Infeksjon" = n),  
        by.x  = "maaned", 
        by.y = "maaned") %>% 
...
if (params$tableFormat == "html") {
  avdKompTot %>% dplyr::select(-aar) %>% 
    ...
    kableExtra::row_spec(row=12, hline_after =  TRUE) %>%
    kableExtra::row_spec(row=13, hline_after =  TRUE) %>%
    kableExtra::row_spec(row=dim(avdKompTot)[1]-1, hline_after =  TRUE) %>%
    kableExtra::pack_rows(group_label = as.character(periode_data$sisteHeleYear),
                          start_row = 1, 
                          end_row = 13) %>% 
    kableExtra::pack_rows(group_label = as.character(periode_data$nyesteRegYear),
...
Bredden på boksen kalles intervartil bredde (IQR). 
...
```{r figVentetid, fig.cap = paste0("Ventetid for planlagte prosedyrer på aortaklaff er beregnet som antall dager mellom dato for screeningbeslutning og prosedyredatoen. Prosedyrene er fra tidsrommet fra ", rangeProsDato, "."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth"}
...
1.5 x IQR over øvre kvartil. 
Punktene utenfor greinene viser uteliggere,
det vil si forløp med usedvanlig kort eller lang liggetiden.  
Merk at datovariablene for innneværende år ikke nødvendigvis er filvasket.

<br/><br/>

```{r figLiggetid, fig.cap = paste0("Liggetid for prosedyrer på aortaklaff er beregnet som antall dager mellom prosedyredatoen og utskrivelsesdasto. Negative liggetider, eller liggetider over 30 dager er fjernet. Prosedyrene er fra ", rangeProsDato, ". Merk at antall prosedyrer i hver gruppe kan være svært ulikt."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth"}
...
pasienten ikke allerede har pacemaker. Når vi rapporterer fra kvalitets-
indikatoren krever vi minst 50% ferdigstillelse for å regne ut 
måloppnåelse. 
<br/><br/>
...
cap <- paste0("Antall prosedyrer med Pacemakerbehov etter innsetting av ", 
              ...
              "som avdød på lab er fjernet fra datagrunnlaget.")
...
fromDate <- as.Date(paste0(periode_data$nyesteRegYear - 10, 
                           "-01-01"), 
                    format = "%Y-%m-%d")
...
```{r figalderkjonn, fig.cap = paste0("Pasienten alder ved prosedyren per i år fra ", fromDate," til ", toDate, ". Merk at antall prosedyrer i hver gruppe kan være svært ulikt."),  fig.width = 8, fig.height = 5,  fig.pos = "H", fig.align = "center", out.width = "\\textwidth"}
