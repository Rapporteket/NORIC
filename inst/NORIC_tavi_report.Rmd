...
labKomp_fjor[labKomp_fjor == 0] <-  " - "
labKomp_fjor[is.na(labKomp_fjor)] <- " - "
```
...
labKomp_aar[labKomp_aar == 0] <-  " - "
labKomp_aar[is.na(labKomp_aar)] <- " - "
```
...
# # align <- c("r", rep("c", dim(labKompTot)[2] - 2))
# # col_nams <- c("", names(labKompTot[-1]))
# 
# 
# 
# if (params$tableFormat == "html") {
#   knitr::kable(x = labKompTot %>% select(-aar),
#                caption = cap) %>% 
...
#     kableExtra::row_spec(row=12, hline_after =  TRUE) %>%
#     kableExtra::row_spec(row=13, hline_after =  TRUE) %>%
#     kableExtra::row_spec(row=dim(labKomp_fjor)[1]-1, hline_after =  TRUE) %>%
#     kableExtra::pack_rows(group_label = as.character(periode_data$sisteHeleYear),
#                           start_row = 1,
#                           end_row = 13)
#   # %>%
...
fig_df_kompAvd <- AK %>%
  ...
  dplyr::mutate(
    type_komplikasjon = case_when(
      ...
      # name %in% "AvdkompBloding_fatal" ~ "Blødning livstruende/fatal", 
      name %in% "AvdKompInfeksjon" ~ "Noen form for infeksjon(!)", 
    ...
    gradering = "Alvorlige komplikasjoner på avdeling") %>% 
...
rekke_avd <- rbind(
  fig_df_kompAvd %>% 
    dplyr::filter(name != "AvdKompAnnenKomp",
                  aar %in% periode_data$sisteHeleYear,
