...
nTrykkmaalingAP <- aP_nasjonalt %>%
  ...
  dplyr::mutate(
    ...
    utfortTrykkmaaling = ifelse(Totalt %in% "N",
                                yes = NA,
                                no = ja),
...
tabTrykkmaalingAP <- timeTableMonth %>%
  ...
  dplyr::rename("Måned" = maaned,
                "Utført trykkmåling" = utfortTrykkmaaling) %>%
  replace(is.na(.), "-")
...
nStentVH <- aP_nasjonalt %>%
  ...
  dplyr::mutate(
    ...
    utfortIvusOct = ifelse(Totalt %in% "N",
                           yes = NA,
                           no = ja),
...
tabStentVH <- timeTableQuarter %>%
  ...
  dplyr::rename("Utført IVUS/OCT" = utfortIvusOct) %>%
  replace(is.na(.), "-")
