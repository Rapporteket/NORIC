...
cap <- paste0("Antall og andel ferdigstilte komplikasjonsskjemaer etter måned.", 
...
tabVentetid24t <- timeTableMonth %>% 
  ...
  dplyr::rename(
    "Måned" = .data$maaned,
    "Antall komplette data av datagrunnlaget" = .data$komplette_tekst,
    "Antall NSTEMI innen 24t av komplette" = .data$nstemi_24t_tekst) %>%
  replace(is.na(.), "-")
...
tabVentetid72t <- timeTableMonth %>% 
  ...
  dplyr::rename(
    "Måned" = .data$maaned,
    "Antall komplette data av datagrunnlaget" = .data$komplette_tekst,
    "Antall NSTEMI innen 72t av komplette" = .data$nstemi_72t_tekst) %>%
  replace(is.na(.), "-")
...
cap <- paste0(
  "Antall og andel pasienter per opphold der det er dokumentert ",
  ...
  "Måneder der andel ferdigstilte prosedyreskjemaer er mindre enn 50\\% ", 
  "blir markert med stjerne og vi regner ikke ut andel forløp med ", 
  "foreskriving av kolesterolsenkende for disse.")
...
tabBlodfMedAP <- timeTableMonth %>% 
  ...
  dplyr::rename(
    "Måned" = .data$maaned,
    "Antall ferdigstilte av datagrunnlaget" = .data$komplette_tekst,
    "Antall med blodfortynnende" = .data$blodf_tekst) %>%
  replace(is.na(.), "-")
...
tabKolestMedAP <- timeTableMonth %>% 
  ...
  dplyr::rename(
    "Måned" = .data$maaned,
    "Antall ferdigstilte av datagrunnlaget" = .data$komplette_tekst,
    "Antall med kolesterolsenkende" = .data$kolest_tekst) %>%
  replace(is.na(.), "-")
...
```{r tab_pacemakerBehov, results='asis', warning=FALSE}
if (dim_ak_lokalt[1] > 10 & aK_nasjonalt %>% nrow() > 0) {
  ...
  cap <- paste0(
    ...
    "Kun observasjoner der prosedyreskjemaet er ferdigstilt er tatt med. ", 
  ...
  tab_pacemakerbehov <- timeTableQuarterAK %>% 
    ...
    dplyr::rename(
      "Kvartal" = .data$kvartal,
      "Antall ferdigstilte prosedyreskjemaer" = .data$komplette_tekst,
      "Antall implanterte perm. pacemaker" = .data$pacemaker_tekst) %>%
    replace(is.na(.), "-")
