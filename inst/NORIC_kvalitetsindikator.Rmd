...
nyeste_registrering_ap <- aP_nasjonalt %>% 
  ...
  # Dersom nyeste registrering er etter dags dato, brukes heller dags dato
  # som refereansedatoer for tabeller/figurer 
  mutate(nyesteReg = min(as.Date(Sys.time()), 
                         .data$nyesteReg_max), 
...
nyeste_registrering_ak <- aK_nasjonalt %>% 
  summarise(nyesteReg_max = max(.data$ProsedyreDato)) %>% 
  # Dersom nyeste registrering er etter dags dato, brukes heller dags dato
  # som referansedato for tabeller/figurer
  mutate(nyesteReg = min(as.Date(Sys.time()), 
                         .data$nyesteReg_max))
```
...
prep_figVentetid24t_year <-
  rbind(as.data.frame(nstemiAP_fra2017_nasjonalt),
        nstemiAP_fra2017_lokalt) %>%
  dplyr::select(.data$Sykehus,
                .data$Year,
                .data$name,
...
```{r fig_ventetid24T_overflyttet, warning=FALSE, fig.width = 8, fig.height = 4, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.cap=paste0("Andel forløp med maks 24 timer ventetid fra 2017 til siste hele år. Analysene inkluderer pasienter overflyttet fra annet sykehus. Vannrett rød stiplet linje markerer forventet måloppnåelse. Søylen vises ikke for sykehuset for år med færre enn 50\\% komplette data eller færre enn 5 overflyttede forløp.")}
