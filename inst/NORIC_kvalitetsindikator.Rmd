...
Indikatoren er basert på pasientgruppen som sendes direkte til PCI-behandlig og 
...
pasientgruppen. Tabell \@ref(tab:tabSTEMI120min) viser måloppnåelse per måned for 
`r params$hospitalName` og nasjonalt (Mål $\geq$ 80%). 



Tabell \@ref(tab:tabSTEMI120min) viser 
måloppnåelse per måned for `r params$hospitalName` og nasjonalt (Mål $\geq$ %). 
...
if (dim_ak_lokalt[1] > 10 & aK_nasjonalt %>% nrow() > 0) {
  ...
  prep_tab_pacemaker_lokalt <- aK_nasjonalt %>%
    dplyr::filter(.data$AvdRESH == params$reshID) %>%
    dplyr::filter(.data$indik_ak_pacemakerbehov_data  == "ja") %>%
    janitor::tabyl(.data$kvartal,
                   .data$indik_ak_pacemakerbehov,
                   show_missing_levels = TRUE ) %>%
    janitor::adorn_totals("col", name = "Total") %>%
    dplyr::mutate(
  ...
  prep_tab_pacemaker_nasjonalt <- aK_nasjonalt %>%
    dplyr::filter(.data$indik_ak_pacemakerbehov_data  == "ja",
                  .data$indik_ak_pacemakerbehov %in% c("ja", "nei")) %>%
    janitor::tabyl(.data$kvartal,
                   .data$indik_ak_pacemakerbehov,
                   show_missing_levels = TRUE ) %>%
    janitor::adorn_totals("col", name = "Total") %>%
