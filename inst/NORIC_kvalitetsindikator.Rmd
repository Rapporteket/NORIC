...
if(length(rapbase::listStagingData(registryName = params$registryName))>0){
  if(rapbase::listStagingData(registryName = params$registryName) %in% "staging_nasjonal_ki"){
    valid_staging_data <- TRUE
  }
}
...
if(valid_staging_data) {
  ...
  aP_nasjonalt %<>% 
    dplyr::filter(
      .data$ProsedyreDato >= as.Date("2018-01-01", format = "%Y-%m-%d"), 
      .data$ProsedyreDato <= periode_data$siste_dato)
  
  aK_nasjonalt %<>% 
    dplyr::filter(
      .data$ProsedyreDato >= periode_data$forste_dato_ak_ss, 
      .data$ProsedyreDato <= periode_data$siste_dato)

  } else{ 
 
  # HENTE DATASETTENE (SQL-spørring)
  ...
  # VELGE KUN DE VARIABLENE VI TRENGER FRA HJELPEDATASETTENE
  sS_nasjonalt %<>%
  ...
  # BEARBEIDE OG UTLEDE NYE VARIABLER
  aP_nasjonalt %<>%
    dplyr::select(
      .data$AvdRESH,
      .data$Sykehusnavn,
  ...
  # _BEARBEIDE OG UTLEDE NYE VARIABLER
  aK_nasjonalt %<>%
    dplyr::select(
      .data$ProsedyreDato,
...
timeTableYear <- data.frame(
  aar = 2018:periode_data$sisteHeleYear)
```
...
Figur \@ref(fig:figVentetid24TAarSamlet) viser måloppnåelse for alle forløp i 
datagrunnlaget, 
figur \@ref(fig:figVentetid24TAarDirekte) for direkte innlagte pasienter og
figur \@ref(fig:figVentetid24TAarOverflyttet) for overflyttede pasienter. 
Figurene \@ref(fig:figVentetid24TSamletAar), 
\@ref(fig:figVentetid24TDirekteAar) og \@ref(fig:figVentetid24TOverflyttetAar)
viser årlig måloppnåelse for alle forløp i datagrunnlaget, kun direkte innlagte
pasienter og kun for overførte pasienter. 
...
```{r figVentetid24TAarSamlet, warning=FALSE, fig.width = 8, fig.height = 4, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.cap = cap}




cap <- paste0("Andel pasienter med indikasjon NSTEMI som innen 24 timer etter innleggelse i sykehus har blitt utredet med invasiv koronar angiografi. Analysene inkluderer både pasienter innlagt direkte på PCI-sykehus og pasienter overflyttet fra annet sykehus. Vannrett rød stiplet linje markerer forventet måloppnåelse. Per måned, fra hele siste året til og med siste hele måned. Måneder der sykehuset har færre enn 50\\% komplette data eller færre enn 5 forløp vises ikke i diagrammet.")

fig1 <- prep_fig_ventetid24t %>%
  dplyr::filter(name %in% "Samlet")
...
```{r figVentetid24TAarDirekte, warning=FALSE, fig.width = 8, fig.height = 4, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.cap=cap}

cap <- "Andel pasienter med indikasjon NSTEMI som innen 24 timer etter innleggelse i sykehus har blitt utredet med invasiv koronar angiografi.  Analysene inkluderer pasienter innlagt direkte på PCI-sykehus. Vannrett rød stiplet linje markerer forventet måloppnåelse. Per måned, fra hele siste året til og med siste hele måned. Måneder der sykehuset har færre enn 50\\% komplette data eller færre enn 5 forløp vises ikke i diagrammet."

prep_fig_ventetid24t %>%
  dplyr::filter(name %in% "Direkte") %>%
  ggplot2::ggplot(aes(x = .data$maaned,
                      y = .data$value,
                      group = .data$Sykehus)) +
...
```{r figVentetid24TAarOverflyttet, warning=FALSE, fig.width = 8, fig.height = 4, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.cap= cap}

cap <- "Andel pasienter med indikasjon NSTEMI som innen 24 timer etter innleggelse i sykehus har blitt utredet med invasiv koronar angiografi.  Analysene inkluderer pasienter overflyttet fra annet sykehus. Vannrett rød stiplet linje markerer forventet måloppnåelse. Per måned, fra hele siste året til og med siste hele måned. Måneder der sykehuset har færre enn 50\\% komplette data eller færre enn 5 forløp vises ikke i diagrammet."


prep_fig_ventetid24t %>%
  dplyr::filter(name %in% "Overflyttet") %>%
  ggplot2::ggplot(aes(x = .data$maaned,
                      y = .data$value,
                      group = .data$Sykehus)) +
...
```{r figVentetid24TSamletAar, warning=FALSE, fig.width = 8, fig.height = 4, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.cap=cap}


cap <- "Andel pasienter med indikasjon NSTEMI som innen 24 timer etter innleggelse i sykehus har blitt utredet med invasiv koronar angiografi.  Analysene inkluderer både pasienter innlagt direkte på PCI-sykehus og pasienter overflyttet fra annet sykehus. Vannrett rød stiplet linje markerer forventet måloppnåelse. Søylen vises ikke for sykehuset for år med færre enn 50\\% komplette data eller færre enn 5 forløp."



fig_samlet <- prep_figVentetid24t_year %>%
  dplyr::filter(.data$name %in% "Samlet")
...
fig_samlet  %>% ggplot2::ggplot(
  ...
  ggplot2::geom_text(
    ...
    vjust = 1.2,
    size = 3,
    colour =  ifelse(fig_samlet$Sykehus %in% "Nasjonalt",
                     "white", "black")) +
  ggplot2::geom_hline(yintercept = 0.5, colour = "red", linetype = "dashed")

```

```{r figVentetid24TDirekteAar, warning=FALSE, fig.width = 8, fig.height = 4, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.cap=cap }
...
fig_direkte <- prep_figVentetid24t_year %>%
  dplyr::filter(.data$name %in% "Direkte")
...
```{r figVentetid24TOverflyttetAar, warning=FALSE, fig.width = 8, fig.height = 4, fig.pos = "H", fig.align = "center", out.width = "\\textwidth", fig.cap=cap}

cap <- "Andel pasienter med indikasjon NSTEMI som innen 24 timer etter innleggelse i sykehus har blitt utredet med invasiv koronar angiografi. Analysene inkluderer pasienter overflyttet fra annet sykehus. Vannrett rød stiplet linje markerer forventet måloppnåelse. Søylen vises ikke for sykehuset for år med færre enn 50\\% komplette data eller færre enn 5 direkte forløp."


fig_overflyttet <- prep_figVentetid24t_year %>%
  dplyr::filter(.data$name %in% "Overflyttet")
...
if (params$reshID != 106944) {
  ...
  cap <- paste0(
    ...
    "regnes ikke ut for disse. (Målnivå: 80 \\%)")
...
cap <- paste0(
  ...
  "Måneder med mindre enn 5 ferdigstilte observasjoner er markert med N.")
