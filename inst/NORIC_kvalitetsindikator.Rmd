...
if(length(rapbase::listStagingData(registryName = params$registryName)) > 0) {
  ...
  stagingData <- rapbase::mtimeStagingData(registryName = params$registryName) %>% 
    as.data.frame() %>% 
  ...
  valid_staging_data <- ifelse(
    test = as.numeric(difftime(time1 = stagingData$date[1],
                               time2 = Sys.Date(),
                               units = "days")) < 2,
    yes = TRUE,
    no = FALSE)
  nyeste_staging_data <- stagingData$name[1]
