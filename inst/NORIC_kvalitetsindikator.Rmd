...
valid_staging_data <- dplyr::case_when(
  
  # Staging data finnes, og har riktig navn
  length(rapbase::listStagingData(registryName = params$registryName))> 0 &
    rapbase::listStagingData(registryName = params$registryName) %in% c(
      "staging_nasjonal_ki") ~ TRUE,
  
  # Staging data finnes ikke
  length(rapbase::listStagingData(registryName = params$registryName)) == 0 ~ 
    FALSE, 
