...
aP_nasjonalt %<>%
  ...
  dplyr::mutate(
    maaned = as.factor(format(x = ProsedyreDato, format = "%Y-%m")),
    aar = lubridate::year(ProsedyreDato),
    kvartal = paste0(lubridate::year(ProsedyreDato),
                     " Q",
                     lubridate::quarter(ProsedyreDato,
                                        with_year = FALSE))) %>%
  dplyr::mutate(admissionType = dplyr::case_when(
    OverflyttetFra %in% "Annet sykehus" ~ "Overflyttet",
...
timeTableQuarter <- data.frame(
  KvartalDatoStart = rev(seq(periode_data$kvartal_komplett_start, 
                             ...
                             length.out = 5))) %>% 
  dplyr::transmute(
    kvartal = paste0(lubridate::year(KvartalDatoStart), 
                     " Q", 
                     lubridate::quarter(KvartalDatoStart, 
                                        with_year = FALSE)))
```
...
nyeste_registrering_ak <- aK_nasjonalt %>% 
  dplyr::summarise(nyesteReg_max = max(ProsedyreDato)) %>% 
...
dim_ak_lokalt <- aK_nasjonalt %>% 
  dplyr::filter(AvdRESH %in% params$reshId) %>% 
  dim(.)
```
...
`r aP_nasjonalt %>% dplyr::select(AvdRESH) %>% dplyr::distinct() %>% nrow()` 
...
ferdigstilt_lokalt_fjor <- merge(
  ...
  dplyr::transmute(
    ...
    Prosent = paste0(round(ja / Totalt * 100, 1), 
                     " %"),
    
    Totalt = ifelse(Totalt < 5,
                    yes = "N",
                    no = Totalt),
    
    Ferdigstilt = ifelse(Totalt %in% "N",
                         yes = NA,
                         no = ja),
    ...
    Prosent = ifelse(Totalt %in% "N",
                     yes = NA,
                     no = Prosent)) %>% 
  dplyr::select(maaned, Totalt, Ferdigstilt, Prosent) %>% 
  dplyr::mutate(aar = periode_data$sisteHeleYear) %>% 
  dplyr::relocate("aar", .before = "maaned")


#Nasjonalt i fjor
ferdigstilt_nasjonalt_fjor <- merge(
  x = timeTableMonth_fjor,
  y = aP_nasjonalt %>% 
    dplyr::filter(indik_komplik_ferdig_data %in% "ja") %>% 
    janitor::tabyl(maaned,
  ...
  janitor::adorn_totals("row", name = paste0("Totalt i ",
                                             periode_data$sisteHeleYear)) %>% 
  dplyr::transmute(
    maaned = as.factor(maaned), 
    Nasjonalt = paste0(round(ja / Totalt * 100, 1), " %"))


#Tabell i fjor
tabFerdigtiltKompl_fjor <- dplyr::left_join(ferdigstilt_lokalt_fjor,
                                            ferdigstilt_nasjonalt_fjor,
                                            by = c("maaned")) %>%
  replace(is.na(.), "-")
...
# Nasjonalt i år
ferdigstilt_nasjonalt_aar <- merge(
  x = timeTableMonth_aar,
  y = aP_nasjonalt %>% 
    dplyr::filter(indik_komplik_ferdig_data %in% "ja") %>% 
    janitor::tabyl(maaned,
...
#Tabell i år
tabFerdigtiltKompl_aar <- dplyr::left_join(ferdigstilt_lokalt_aar,
                                            ferdigstilt_nasjonalt_aar,
                                            by = c("maaned")) %>%
  replace(is.na(.), "-")

# Tabell samlet
tabFerdigtiltKompl <- rbind(
  tabFerdigtiltKompl_fjor,
  tabFerdigtiltKompl_aar)
...
align <- c("l", rep("c", dim(tabFerdigtiltKompl)[2] - 1))
digs <-  rep(0, dim(tabFerdigtiltKompl)[2])

 tabFerdigtiltKompl %>% dplyr::select(-aar) %>% rename(" " = "maaned") %>% 
    ...
    kableExtra::row_spec(row=dim(tabFerdigtiltKompl)[1]-1, hline_after =  TRUE) %>%
    ...
    kableExtra::pack_rows(group_label = as.character(periode_data$nyesteRegYear),
                          start_row = 14, 
                          end_row = dim(tabFerdigtiltKompl)[1])
```
...
# Lokalt i fjor
nstemi_24t_lokalt_fjor <- merge(
  x = timeTableMonth_fjor,
  y = aP_nasjonalt %>%
  dplyr::filter(AvdRESH %in% params$reshID) %>%
  dplyr::filter(indik_nstemi_angio_innen24t_data %in% "ja") %>%
  janitor::tabyl(maaned,
  ...
  dplyr::mutate(
    ...
    komplette_antall = ja + nei,
    komplette_prosent =  komplette_antall / Total,
    komplette_tekst = dplyr::if_else(
      komplette_prosent < 0.50 | komplette_antall < 5,
      true = paste0(komplette_antall, " av ", Total, "*"),
      false = paste0(komplette_antall, " av ", Total),
      missing = NA_character_),
    
    # Blandt komplette: Antall NTSTEMI innen 24 t ,
    nstemi_24t_tekst = dplyr::if_else(
      komplette_prosent < 0.50 | komplette_antall < 5,
      true = paste0(ja, " av ", komplette_antall, " (-%)"),
      false = paste0(ja, " av ", komplette_antall, " (",
                     round(100 * ja / komplette_antall, 1), "%)"),
      missing = NA_character_)) %>%
  dplyr::select(maaned,
                komplette_tekst,
                nstemi_24t_tekst) %>% 
...
# Nasjonalt i fjor
nstemi_24t_nasjonalt_fjor <- 
  merge(
    x = timeTableMonth_fjor,
    y = aP_nasjonalt %>%
      dplyr::filter(indik_nstemi_angio_innen24t_data %in% "ja",
                    indik_nstemi_angio_innen24t %in% c("ja", "nei")) %>%
      janitor::tabyl(maaned,
                     indik_nstemi_angio_innen24t,
                     show_missing_levels = TRUE) %>%
      janitor::adorn_totals("col", name = "Totalt"),
  ...
  dplyr::transmute(
    maaned = maaned,
    Nasjonalt = paste0(round(100 * ja / Totalt, 1), "%"))


nstemi_24t_fjor <- dplyr::left_join(nstemi_24t_lokalt_fjor,
                                    nstemi_24t_nasjonalt_fjor,
                                    by = c("maaned")) %>%
  replace(is.na(.), "-")
...
nstemi_24t_aar <- dplyr::left_join(nstemi_24t_lokalt_aar,
                                   nstemi_24t_nasjonalt_aar,
                                   by = c("maaned")) %>%
  replace(is.na(.), "-")

# Forberede tabell, bruke kompletthet og ventetid
tabVentetid24t <- rbind(
  nstemi_24t_fjor,
  nstemi_24t_aar)


align <- c("l", rep("c", dim(tabVentetid24t)[2] - 1))
digs <-  rep(0, dim(tabVentetid24t)[2])

 tabVentetid24t %>% dplyr::select(-aar) %>% rename(" " = "maaned") %>% 
    knitr::kable(format = params$tableFormat,
               caption = cap,
               booktabs = TRUE,
               align = align,
    ...
    kableExtra::row_spec(row=12, hline_after =  TRUE) %>%
    kableExtra::row_spec(row=13, hline_after =  TRUE)%>%
    kableExtra::row_spec(row=dim(tabVentetid24t)[1]-1, hline_after =  TRUE) %>%
    kableExtra::pack_rows(group_label = as.character(periode_data$sisteHeleYear),
                          start_row = 1,
                          end_row = 13) %>% 
    kableExtra::pack_rows(group_label = as.character(periode_data$nyesteRegYear),
                          start_row = 14, 
                          end_row = dim(tabVentetid24t)[1])
...
nstemi_komplette <- aP_nasjonalt %>%
  dplyr::filter(AvdRESH %in% params$reshId,
                indik_nstemi_angio_innen24t_data %in% "ja") %>%
  janitor::tabyl(maaned,
                 indik_nstemi_angio_innen24t,
                 show_missing_levels = TRUE) %>%
  janitor::adorn_totals("col", name  = "Totalt") %>%
  transmute(
    maaned = maaned,
    komplette_antall = (ja + nei),
    komplette_andel = komplette_antall / Totalt)
...
prep_figVentetid24t_lokalt <- aP_nasjonalt %>%
  ...
  dplyr::filter(indik_nstemi_angio_innen24t_data %in% "ja",
                indik_nstemi_angio_innen24t %in% c("ja", "nei")) %>%
  dplyr::filter(ProsedyreDato >= periode_data$forste_dato,
                ProsedyreDato <= periode_data$siste_dato) %>%
  
  #Samlet *(Direkte og overflyttet)
  dplyr::group_by(maaned) %>%
  dplyr::mutate(
    innen_24t_samlet = sum(indik_nstemi_angio_innen24t %in% "ja",
                           na.rm = TRUE)/n(),
    n_samlet = n()) %>%
  dplyr::ungroup() %>%
  
  # per admission type (Direkte / OVerflyttet)
  dplyr::group_by(maaned, admissionType) %>%
  dplyr::mutate(
    innen_24t_type = sum(indik_nstemi_angio_innen24t %in% "ja",
                         na.rm = TRUE)/n()) %>%
  dplyr::ungroup() %>%
  ...
  dplyr::mutate(name = dplyr::case_when(
    ...
    admissionType %in% "Direkte" &
      Type %in% "innen_24t_type" ~ "Direkte",
    
    admissionType %in% "Overflyttet" &
      Type %in% "innen_24t_type" ~ "Overflyttet",
    
    admissionType %in% "Direkte" &
      Type %in% "innen_24t_samlet" ~ "Samlet",
  ...
  antall_komplette = dplyr::case_when(
    Type %in% "innen_24t_type" ~ n,
    Type %in% "innen_24t_samlet" ~ n_samlet
  )) %>%
  dplyr::select(maaned,
                name,
                value,
                antall_komplette) %>%
  dplyr::group_by(maaned, name) %>%
  dplyr::distinct()
...
prep_figVentetid24t_lokalt %<>%
  dplyr::left_join(.,
                   nstemi_komplette %>%
                     select(maaned, komplette_andel),
                   by = "maaned") %>%
  dplyr::mutate(
    value = ifelse(komplette_andel < 0.5 | antall_komplette < 5,
                   yes = NA_real_,
                   no = value),
    Sykehus = params$hospitalName) %>%
  dplyr::select(maaned, name, value, Sykehus)
...
prep_figVentetid24t_nasjonalt <- aP_nasjonalt %>%
  ...
  dplyr::mutate(name = dplyr::case_when(
    ...
    admissionType %in% "Overflyttet" &
      Type %in% "innen_24t_samlet" ~ "Samlet"  )) %>%
  dplyr::select(maaned, name, value) %>%
  dplyr::group_by(maaned, name) %>%
  dplyr::distinct() %>%
...
prep_fig_ventetid24t <- rbind(prep_figVentetid24t_nasjonalt,
  ...
  dplyr::select(Sykehus,
                ...
                name,
                value) %>%
  dplyr::mutate(Sykehus = factor(Sykehus,
...
ggplot2::ggplot(data = prep_fig_ventetid24t,
                aes(x = maaned,
                    y = value,
                    group = Sykehus)) +
  ggplot2::geom_line(aes(colour = Sykehus)) +
  ggplot2::geom_point(aes(colour = Sykehus))  +
...
nstemiAP_fra2018_lokalt <- aP_nasjonalt %>%
  ...
  dplyr::filter(indik_nstemi_angio_innen24t_data %in% "ja",
                indik_nstemi_angio_innen24t %in% c("ja", "nei")) %>%
  dplyr::filter(aar %in% 2018:periode_data$nyesteRegYear) %>%
  ...
  # per admission type (Direkte / OVerflyttet)
  dplyr::group_by(aar, admissionType) %>%
  dplyr::mutate(
    innen_24t_type = sum(indik_nstemi_angio_innen24t %in% "ja",
                         na.rm = TRUE)/n()) %>%
  dplyr::ungroup() %>%
  ...
  dplyr::select(aar, name, value) %>%
  dplyr::group_by(aar, name) %>%
  dplyr::distinct()
...
nstemiAP_fra2018_lokalt %<>%
  ...
  dplyr::transmute(
    aar = as.numeric(aar),
    name = name,
    value = ifelse(komplette_andel < 0.5,
                   yes = NA_real_,
                   no = value))
...
prep_figVentetid24t_year <-
  ...
  dplyr::mutate(Sykehus = factor(Sykehus,
                ...
                value_tekst = ifelse(
                  !is.na(value),
                  yes = paste0(sprintf("%.1f", value*100 ), "%"),
                  no = NA_character_))
...
nstemi_72t_lokalt <- aP_nasjonalt %>%
  dplyr::filter(AvdRESH %in% params$reshId) %>%
  dplyr::filter(indik_nstemi_angio_innen72t_data %in% "ja") %>%
  janitor::tabyl(maaned,
...
nstemi_72t_nasjonalt <- aP_nasjonalt %>%
  dplyr::filter(indik_nstemi_angio_innen72t_data %in% "ja",
                indik_nstemi_angio_innen72t %in% c("ja", "nei")) %>%
  janitor::tabyl(maaned,
                 indik_nstemi_angio_innen72t,
                 show_missing_levels = TRUE) %>%
  janitor::adorn_totals("col", name = "Totalt") %>% 
...
tabVentetid72t <- timeTableMonth %>%
  ...
  dplyr::rename(
    "Måned" = maaned,
    "Komplette av datagrunnlaget" = komplette_tekst,
    "Angio innen 72t av komplette" = nstemi_72t_tekst) %>%
  replace(is.na(.), "-")
...
if (params$reshId != 106944) {
  ...
  stemi_120min_lokalt <- aP_nasjonalt %>%
    dplyr::filter(AvdRESH %in% params$reshId) %>%
    dplyr::filter(indik_stemi_pci_innen2t_data %in% "ja") %>%
    janitor::tabyl(maaned,
  ...
  stemi_120min_nasjonalt <- aP_nasjonalt %>%
    dplyr::filter(indik_stemi_pci_innen2t_data %in% "ja",
                  indik_stemi_pci_innen2t %in% c("ja", "nei")) %>%
    janitor::tabyl(maaned,
                   indik_stemi_pci_innen2t,
                   show_missing_levels = TRUE) %>%
    janitor::adorn_totals("col", name = "Totalt") %>% 
  ...
  tabSTEMI_120min <- timeTableMonth %>%
    ...
    dplyr::rename(
      "Måned" = maaned,
      "Komplette av datagrunnlaget" = komplette_tekst,
      "PCI innen 120min av komplette" = stemi_120min_tekst) %>%
    replace(is.na(.), "-")
...
kransårerøntgen hos pasienter med stabil angina pectoris og der angiografi 
viser minst ett segment med stenosegrad mellom 30 - 99 %. Tabell
...
cap <- paste0(
  ...
  "koronarsykdom og der angiografi viser minst ett segment med ",
  "stenosegrad mellom 30 - 99 \\% har blitt utført trykkmåling i ",
  "form av enten fractional flow reserve (FFR), ",
  ...
  "Med 'komplette' menes forløp der variabelen 'Annen diagnostikk' ikke er ",
  "manglende eller ukjent. Måneder med mindre enn 50 \\% komplette data ",
  "eller færre enn 5 forløp blir markert med stjerne og andel forløp med ",
  "utført trykkmåling beregnes ikke.")


#Lokal
nTrykkmaalingAP <- aP_nasjonalt %>%
  dplyr::filter(AvdRESH %in% params$reshId) %>%
  dplyr::filter(indik_trykkmaaling_data %in% "ja") %>%
  dplyr::select(maaned,
                indik_trykkmaaling) %>%
  janitor::tabyl(maaned,
                 indik_trykkmaaling,
  ...
  janitor::adorn_totals("col", name = "Totalt") %>%
  dplyr::mutate(
    # Komplette er alle som er ja/nei, men ikke manglende
    komplette_antall = ja + nei,
    komplette_prosent =  komplette_antall / Totalt,
    komplette_tekst = dplyr::if_else(
      komplette_prosent < 0.50 | komplette_antall < 5,
      true = paste0(komplette_antall, " av ", Totalt, "*"),
      false = paste0(komplette_antall, " av ", Totalt),
      missing = NA_character_),
    
    # Blandt komplette: Utført trykkmåling ,
    utfortTrykkmaaling_tekst = dplyr::if_else(
      komplette_prosent < 0.50 | komplette_antall < 5,
      true = paste0(ja, " av ", komplette_antall, " (-%)"),
      false = paste0(ja, " av ", komplette_antall, " (",
                     round(100 * ja / komplette_antall, 1), "%)"),
  ...
  dplyr::select(maaned,
                komplette_tekst,
                utfortTrykkmaaling_tekst)

#Nasjonal
nTrykkmaalingAP_nasj <- aP_nasjonalt %>%
  dplyr::filter(indik_trykkmaaling_data %in% "ja",
                indik_trykkmaaling %in% c("ja", "nei")) %>%
  janitor::tabyl(maaned,
                 indik_trykkmaaling,
                 show_missing_levels = TRUE) %>%
  janitor::adorn_totals("col", name = "Totalt") %>%
...
tabTrykkmaalingAP <- timeTableMonth %>%
  ...
  dplyr::rename(
    "Måned" = maaned,
    "Komplette av datagrunnlaget" = komplette_tekst,
    "Utført trykkmåling av komplette" = utfortTrykkmaaling_tekst) %>%
  replace(is.na(.), "-")
...
cap <- paste0(
  "Antall og andel prosedyrer der det er brukt supplerende ",
  "billeddiagnostike metoder (IVUS og/eller OCT). ",
  "Med 'komplette' menes forløp der variabelen 'Annen diagnostikk' ikke er ",
  "manglende eller ukjent. Kvartaler med mindre enn 50 \\% komplette data ",
  "eller færre enn 5 forløp blir markert med stjerne og andel forløp med ",
  "supplerende bildeframstilling i tilknytning til PCI av venstre hovedstamme ",
  "beregnes ikke. ")
...
nStentVH <- aP_nasjonalt %>%
  dplyr::filter(AvdRESH %in% params$reshId) %>%
  dplyr::filter(indik_ivus_oct_v_stent_lms_data %in% "ja") %>%
  dplyr::select(kvartal,
                indik_ivus_oct_v_stent_lms) %>%
  janitor::tabyl(kvartal,
                 indik_ivus_oct_v_stent_lms,
                 show_missing_levels = TRUE) %>%
  janitor::adorn_totals("col", name = "Totalt") %>%
  dplyr::mutate(
    ...
    # Blandt komplette: 
    #   Bildeframstilling i tilknytning til PCI av venstre hovedstamme.
   utfortIvusOct_tekst = dplyr::if_else(
      komplette_prosent < 0.50 | komplette_antall < 5,
      true = paste0(ja, " av ", komplette_antall, " (-%)"),
      false = paste0(ja, " av ", komplette_antall, " (",
                     round(100 * ja / komplette_antall, 1), "%)"),
...
nStentVH_nasjonalt <- aP_nasjonalt %>%
  dplyr::filter(indik_ivus_oct_v_stent_lms_data %in% "ja",
                indik_ivus_oct_v_stent_lms %in% c("ja", "nei")) %>%
  janitor::tabyl(kvartal,
                 indik_ivus_oct_v_stent_lms,
                 show_missing_levels = TRUE) %>%
  janitor::adorn_totals("col", name = "Totalt") %>%
...
tabStentVH <- timeTableQuarter %>%
  ...
  dplyr::rename(
    "Kvartal" = kvartal,
    "Komplette av datagrunnlaget" = komplette_tekst,
    "Utført IVUS/OCT av komplette" = utfortIvusOct_tekst) %>%
  replace(is.na(.), "-")
...
blodfortynnende_lokalt <- aP_nasjonalt %>%
  dplyr::filter(AvdRESH %in% params$reshId) %>%
  dplyr::filter(indik_blodfortynnende_data %in% "ja") %>%
  janitor::tabyl(maaned,
...
blodfortynnende_nasjonalt <- aP_nasjonalt %>%
  dplyr::filter(indik_blodfortynnende_data %in% "ja",
                indik_blodfortynnende %in% c("ja", "nei")) %>%
  janitor::tabyl(maaned,
                 indik_blodfortynnende,
                 show_missing_levels = TRUE) %>%
  janitor::adorn_totals("col", name = "Totalt") %>%
...
tabBlodfMedAP <- timeTableMonth %>%
  ...
  dplyr::rename(
    "Måned" = maaned,
    "Ferdigstilte av datagrunnlaget" = komplette_tekst,
    "Med blodfortynnende av ferdigstilte" = blodf_tekst) %>%
  replace(is.na(.), "-")
...
kolesterolsenkende_lokalt <- aP_nasjonalt %>%
  dplyr::filter(AvdRESH %in% params$reshId) %>%
  dplyr::filter(indik_kolesterolsenkende_data %in% "ja") %>%
  janitor::tabyl(maaned,
...
kolesterolsenkende_nasjonalt <- aP_nasjonalt %>%
  dplyr::filter(indik_kolesterolsenkende_data %in% "ja",
                indik_kolesterolsenkende %in% c("ja", "nei")) %>%
  janitor::tabyl(maaned,
                 indik_kolesterolsenkende,
                 show_missing_levels = TRUE) %>%
  janitor::adorn_totals("col", name = "Totalt") %>%
...
tabKolestMedAP <- timeTableMonth %>%
  ...
  dplyr::rename(
    "Måned" = maaned,
    "Ferdigstilte av datagrunnlaget" = komplette_tekst,
    "Med kolesterolsenkende av ferdigstilte" = kolest_tekst) %>%
  replace(is.na(.), "-")
...
if (dim_ak_lokalt[1] > 10 & aK_nasjonalt %>% nrow() > 0) {
  ...
  prep_tab_pacemaker_nasjonalt <- aK_nasjonalt %>%
    dplyr::filter(indik_pacemakerbehov_data  %in% "ja",
                  indik_pacemakerbehov %in% c("ja", "nei")) %>%
    janitor::tabyl(kvartal,
                   indik_pacemakerbehov,
                   show_missing_levels = TRUE ) %>%
    janitor::adorn_totals("col", name = "Total") %>%
    dplyr::transmute(
      kvartal = kvartal,
      Nasjonalt = paste0(round(100 * ja / Total, 1), "%"))
  ...
  tab_pacemakerbehov <- timeTableQuarterAK %>%
    ...
    dplyr::rename(
      "Kvartal" = kvartal,
      "Ferdigstilte av datagrunnlaget" = komplette_tekst,
      "Implanterte perm. pacemaker av ferdigstilte" = pacemaker_tekst) %>%
    replace(is.na(.), "-")
