...
Tabell \@ref(tab:tabSTEMI120min) viser måloppnåelse per måned for 
`r params$hospitalName` og nasjonalt (Mål $\geq$ 80%). 
`r if (params$reshID == 106944) {"-->"}`

`r if (params$reshID != 106944) {"<!--"}`
Denne indikatoren er ikke regnet ut for `r aP_nasjonalt %>% dplyr::filter(AvdRESH == params$reshID) %>%   dplyr::distinct(Sykehusnavn) %>% dplyr::pull()` da dette sykehuset kun
unntaksvis har STEMI pasienter. 
`r if (params$reshID != 106944) {"-->"}`
